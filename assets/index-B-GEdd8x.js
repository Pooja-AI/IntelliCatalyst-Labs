...
}`},{id:`hybrid-search`,category:`Retrieval`,title:`Hybrid Search`,difficulty:`Intermediate`,time:`~30 min`,description:`Combine dense semantic vectors with sparse BM25 keyword search, then re-rank. Handles both conceptual and exact-match queries.`,tags:[`BM25`,`reranking`,`fusion`],steps:[{label:`Dense Embed`,icon:`🔢`,detail:`Embed all chunks with a dense model for semantic search.`},{label:`Build BM25 Index`,icon:`🔑`,detail:`Build a sparse BM25 index over the same corpus for keyword matching.`},{label:`Dual Retrieval`,icon:`⚡`,detail:`Run both dense (vector) and sparse (BM25) retrieval in parallel.`},{label:`Reciprocal Rank Fusion`,icon:`🔀`,detail:`Merge result lists using RRF: score = Σ 1/(k + rank_i) where k=60.`},{label:`Cross-Encoder Rerank`,icon:`🏆`,detail:`Pass top-20 fused results through a cross-encoder to get a final top-5.`},{label:`Generate`,icon:`✨`,detail:`Use reranked top-5 as context for the LLM.`}],code:`import { BM25 } from "bm25-ts";
...
const answer = await llm.invoke(
  ...
  Question:
  \${query}\`
);`},{id:`metadata-filtering`,category:`Retrieval`,title:`Metadata Filtering`,difficulty:`Advanced`,time:`~20 min`,description:`Restrict retrieval results using metadata such as department, date, author, source, or access level.`,tags:[`metadata`,`filters`,`enterprise-rag`,`retrieval`],steps:[{label:`Attach Metadata`,icon:`🏷️`,detail:`Store metadata with every document chunk.`},{label:`Apply Filters`,icon:`🔒`,detail:`Filter retrieval by attributes.`},{label:`Vector Search`,icon:`🔍`,detail:`Search only within filtered documents.`},{label:`Generate Answer`,icon:`✨`,detail:`Return answers using filtered context.`}],code:`const results = await index.query({
  vector: embedding,
  topK: 10,
  filter: {
    department: "finance",
...
console.log(answer);`},{id:`react-agent`,category:`Foundations`,title:`ReAct Agent`,difficulty:`Beginner`,time:`~20 min`,description:`Reason then Act: the model emits explicit Thought → Action → Observation chains before producing a final answer. Improves traceability.`,tags:[`reasoning`,`chain-of-thought`,`structured`],steps:[{label:`System Prompt`,icon:`📋`,detail:`Instruct the model to always output Thought: / Action: / Observation: blocks before answering. This structures its reasoning visibly.`},{label:`Thought`,icon:`🤔`,detail:`Model reasons about what it knows and what it needs. E.g. 'I need the current stock price before I can compare.'`},{label:`Action`,icon:`⚡`,detail:`Model outputs a structured action: tool name + arguments, parsed by your loop.`},{label:`Observation`,icon:`👁️`,detail:`Your code executes the action and injects the result as Observation: back into the prompt.`},{label:`Repeat or Answer`,icon:`🔄`,detail:`The model continues Thought → Action → Observation until it has enough to produce a final Answer: block.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log("Answer:", answer);`},{id:`plan-execute`,category:`Planning`,title:`Plan & Execute`,difficulty:`Intermediate`,time:`~30 min`,description:`Separate planning from execution: first generate a full task plan, then execute each step with a smaller model. Reduces token cost and improves reliability.`,tags:[`planning`,`multi-step`,`decomposition`],steps:[{label:`Planner LLM`,icon:`🗺️`,detail:`A capable model (e.g. claude-opus-4-6) receives the goal and outputs a structured JSON plan: an ordered list of steps with tool assignments.`},{label:`Parse Plan`,icon:`📋`,detail:`Extract the step list. Validate that each step has a clear action, tool, and expected output field.`},{label:`Execute Steps`,icon:`⚙️`,detail:`A smaller executor model (or direct tool calls) runs each step in sequence, passing outputs as inputs to the next step.`},{label:`Context Passing`,icon:`🔗`,detail:`Maintain a shared context dict. Each step reads from and writes to context, so outputs flow naturally to downstream steps.`},{label:`Replan on Failure`,icon:`🔁`,detail:`If a step fails, optionally call the planner again with the error context to generate a revised sub-plan.`},{label:`Synthesize`,icon:`✨`,detail:`After all steps complete, pass the full context to the LLM to synthesize a final coherent answer.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(result);`},{id:`multi-agent`,category:`Multi-Agent`,title:`Multi-Agent Orchestration`,difficulty:`Advanced`,time:`~60 min`,description:`An orchestrator agent delegates subtasks to specialized sub-agents running in parallel. Each sub-agent has its own tools and system prompt.`,tags:[`orchestration`,`parallelism`,`specialization`],steps:[{label:`Orchestrator`,icon:`🎯`,detail:`A coordinator LLM receives the high-level goal and decomposes it into subtasks, routing each to the appropriate specialist agent.`},{label:`Agent Registry`,icon:`📚`,detail:`Define a registry of sub-agents: each with a name, system prompt, and tool set. The orchestrator picks agents by name.`},{label:`Parallel Dispatch`,icon:`⚡`,detail:`Use Promise.all to run multiple sub-agents concurrently, dramatically reducing total wall-clock time.`},{label:`Sub-Agent Execution`,icon:`🤖`,detail:`Each sub-agent runs its own tool-use loop independently, returning a structured result when done.`},{label:`Result Aggregation`,icon:`🔗`,detail:`The orchestrator receives all sub-agent outputs and synthesizes them into a coherent final response.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(result);`},{id:`reflection-agent`,category:`Advanced`,title:`Reflection & Self-Correction`,difficulty:`Intermediate`,time:`~25 min`,description:`The agent critiques its own output, identifies weaknesses, and iteratively improves until a quality threshold is met or max rounds exhausted.`,tags:[`self-critique`,`iterative`,`quality`],steps:[{label:`Initial Generation`,icon:`📝`,detail:`Generate a first-pass response to the user's request without any external constraints.`},{label:`Critic Pass`,icon:`🔎`,detail:`A separate critic prompt evaluates the output on dimensions like accuracy, completeness, clarity, and tone.`},{label:`Parse Score`,icon:`📊`,detail:`Extract a numeric quality score (1–10) and a list of specific critiques from the critic's response.`},{label:`Refine`,icon:`✏️`,detail:`If score < threshold, pass the original output + critiques back to the generator with instruction to improve.`},{label:`Loop / Stop`,icon:`🔄`,detail:`Repeat up to N rounds. Stop early if score meets threshold or critique indicates no further improvement possible.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log("Improvement history:", history);`},{id:`memory-agent`,category:`Memory`,title:`Agent with Long-Term Memory`,difficulty:`Advanced`,time:`~45 min`,description:`Equip an agent with episodic and semantic memory: store past interactions as embeddings, retrieve relevant context at query time.`,tags:[`memory`,`embeddings`,`persistence`],steps:[{label:`Episodic Store`,icon:`💾`,detail:`After each conversation turn, embed and store the exchange (user msg + agent response) with a timestamp in a vector DB.`},{label:`Memory Retrieval`,icon:`🔍`,detail:`At the start of each turn, embed the current query and retrieve the top-K most semantically similar past episodes.`},{label:`Semantic Summary`,icon:`📝`,detail:`Periodically compress older episodes into a semantic summary to reduce context length without losing key facts.`},{label:`Context Assembly`,icon:`🧩`,detail:`Inject retrieved memories and the semantic summary into the system prompt before the current turn.`},{label:`Tool-Use Loop`,icon:`🔄`,detail:`Run the standard tool-use agent loop with the enriched context, giving the agent access to its own history.`},{label:`Prune & Archive`,icon:`🗂️`,detail:`Implement a TTL or importance score to prune stale memories and archive rarely-accessed episodes.`}],code:`import Anthropic from "@anthropic-ai/sdk";
import { OpenAI } from "openai";
import { Pinecone } from "@pinecone-database/pinecone";
...
console.log(await memoryAgent("What was I working on again?", SESSION_ID)); // Recalls from memory`}],lr=[`All`,`Core Concepts`,`Planning`,`Tools & Actions`,`Memory`,`Multi-Agent`,`Advanced`],ur={Beginner:`#0F6E56`,Intermediate:`#185FA5`,Advanced:`#993C1D`},dr={Beginner:`#E1F5EE`,Intermediate:`#E6F1FB`,Advanced:`#FAECE7`};function fr({steps:e}){let[t,n]=(0,x.useState)(null);return(0,N.jsxs)(`div`,{style:{marginTop:16},children:[(0,N.jsx)(`div`,{style:{display:`flex`,flexWrap:`wrap`,gap:8,alignItems:`center`},children:e.map((r,i)=>(0,N.jsxs)(`div`,{style:{display:`flex`,alignItems:`center`,gap:6},children:[(0,N.jsxs)(`button`,{onClick:()=>n(t===i?null:i),style:{display:`flex`,alignItems:`center`,gap:6,padding:`6px 12px`,borderRadius:20,border:t===i?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:t===i?`#E6F1FB`:`var(--color-background-primary)`,color:t===i?`#185FA5`:`var(--color-text-primary)`,cursor:`pointer`,fontSize:13,fontWeight:t===i?500:400,transition:`all 0.15s`},children:[(0,N.jsx)(`span`,{children:r.icon}),(0,N.jsx)(`span`,{children:r.label})]}),i<e.length-1&&(0,N.jsx)(`span`,{style:{color:`var(--color-text-tertiary)`,fontSize:12},children:`→`})]},i))}),t!==null&&(0,N.jsxs)(`div`,{style:{marginTop:10,padding:`10px 14px`,borderRadius:8,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.6},children:[(0,N.jsxs)(`span`,{style:{fontWeight:500,color:`var(--color-text-primary)`},children:[e[t].label,`: `]}),e[t].detail]})]})}function pr({code:e}){let[t,n]=(0,x.useState)(!1);return(0,N.jsxs)(`div`,{style:{position:`relative`,marginTop:16},children:[(0,N.jsx)(`button`,{onClick:()=>{navigator.clipboard.writeText(e),n(!0),setTimeout(()=>n(!1),1800)},style:{position:`absolute`,top:8,right:8,padding:`4px 10px`,borderRadius:6,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,cursor:`pointer`,fontSize:12,color:`var(--color-text-secondary)`,zIndex:1},children:t?`✓ Copied`:`Copy`}),(0,N.jsx)(`pre`,{style:{margin:0,padding:`14px 16px`,borderRadius:10,overflowX:`auto`,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:12,lineHeight:1.65,fontFamily:`var(--font-mono)`,color:`var(--color-text-primary)`,whiteSpace:`pre`},children:(0,N.jsx)(`code`,{children:e})})]})}function mr({recipe:e,onSelect:t,selected:n}){return(0,N.jsxs)(`div`,{onClick:()=>t(e),style:{padding:`16px 18px`,borderRadius:12,cursor:`pointer`,border:n?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:n?`#061320`:`var(--color-background-primary)`,transition:`all 0.15s`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:6},children:[(0,N.jsx)(`span`,{style:{fontSize:13,color:`var(--color-text-secondary)`,fontWeight:400},children:e.category}),(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,fontWeight:500,background:dr[e.difficulty],color:ur[e.difficulty]},children:e.difficulty})]}),(0,N.jsx)(`div`,{style:{fontWeight:500,fontSize:15,marginBottom:4,color:`var(--color-text-primary)`},children:e.title}),(0,N.jsx)(`div`,{style:{fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.5},children:e.description}),(0,N.jsx)(`div`,{style:{marginTop:10,display:`flex`,gap:6,flexWrap:`wrap`},children:e.tags.map(e=>(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,background:`var(--color-background-tertiary)`,color:`var(--color-text-secondary)`,border:`0.5px solid var(--color-border-tertiary)`},children:e},e))})]})}function hr({recipe:e}){let[t,n]=(0,x.useState)(`steps`);return(0,N.jsxs)(`div`,{style:{padding:`24px`,borderRadius:14,background:`var(--color-background-primary)`,border:`0.5px solid var(--color-border-tertiary)`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:4},children:[(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:e.category}),(0,N.jsx)(`h2`,{style:{margin:`4px 0 6px`,fontSize:22,fontWeight:500},children:e.title})]}),(0,N.jsxs)(`div`,{style:{display:`flex`,gap:8,alignItems:`center`,paddingTop:4},children:[(0,N.jsx)(`span`,{style:{fontSize:12,padding:`3px 10px`,borderRadius:20,fontWeight:500,background:dr[e.difficulty],color:ur[e.difficulty]},children:e.difficulty}),(0,N.jsxs)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:[`⏱ `,e.time]})]})]}),(0,N.jsx)(`p`,{style:{margin:`0 0 20px`,color:`var(--color-text-secondary)`,fontSize:14,lineHeight:1.6},children:e.description}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:4,marginBottom:18,borderBottom:`0.5px solid var(--color-border-tertiary)`,paddingBottom:0},children:[`steps`,`code`].map(e=>(0,N.jsx)(`button`,{onClick:()=>n(e),style:{padding:`8px 16px`,border:`none`,background:`none`,cursor:`pointer`,fontSize:14,fontWeight:t===e?500:400,color:t===e?`var(--color-text-primary)`:`var(--color-text-secondary)`,borderBottom:t===e?`2px solid #185FA5`:`2px solid transparent`,marginBottom:-1,transition:`all 0.12s`},children:e===`steps`?`Pipeline Steps`:`Code`},e))}),t===`steps`&&(0,N.jsx)(fr,{steps:e.steps}),t===`code`&&(0,N.jsx)(pr,{code:e.code})]})}function gr({recipes:e,selected:t,onSelect:n,category:r,setCategory:i,search:a,setSearch:o}){let s=e.filter(e=>{let t=r===`All`||e.category===r,n=e.title.toLowerCase().includes(a.toLowerCase())||e.tags.some(e=>e.toLowerCase().includes(a.toLowerCase()));return t&&n});return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100%`,gap:0},children:[(0,N.jsx)(`div`,{style:{padding:`0 0 16px`},children:(0,N.jsx)(`input`,{type:`text`,placeholder:`Search recipes…`,value:a,onChange:e=>o(e.target.value),style:{width:`100%`,boxSizing:`border-box`,padding:`8px 12px`,borderRadius:8,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,color:`var(--color-text-primary)`,fontSize:13}})}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:6,flexWrap:`wrap`,marginBottom:16},children:lr.map(e=>(0,N.jsx)(`button`,{onClick:()=>i(e),style:{padding:`4px 12px`,borderRadius:20,fontSize:12,cursor:`pointer`,border:r===e?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:r===e?`#E6F1FB`:`var(--color-background-primary)`,color:r===e?`#185FA5`:`var(--color-text-secondary)`,fontWeight:r===e?500:400},children:e},e))}),(0,N.jsx)(`div`,{style:{display:`flex`,flexDirection:`column`,gap:10,overflowY:`auto`,flex:1},children:s.length===0?(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,fontSize:13,padding:`12px 0`},children:`No recipes found.`}):s.map(e=>(0,N.jsx)(mr,{recipe:e,onSelect:n,selected:t?.id===e.id},e.id))})]})}function _r(){return(0,N.jsxs)(`div`,{style:{padding:`20px 32px 16px`,borderBottom:`0.5px solid var(--color-border-tertiary)`,display:`flex`,alignItems:`center`,gap:16},children:[(0,N.jsx)(`div`,{style:{width:40,height:40,borderRadius:10,background:`#E1F5EE`,display:`flex`,alignItems:`center`,justifyContent:`center`,fontSize:20},children:`🤖`}),(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`h1`,{style:{margin:0,fontSize:20,fontWeight:500,letterSpacing:`-0.3px`},children:`Agentic AI Cookbook`}),(0,N.jsx)(`p`,{style:{margin:0,fontSize:13,color:`var(--color-text-secondary)`},children:`End-to-end patterns for building autonomous AI agents`})]}),(0,N.jsx)(`div`,{style:{marginLeft:`auto`,display:`flex`,gap:20},children:[{label:`Recipes`,value:cr.length},{label:`Patterns`,value:lr.length-1}].map(({label:e,value:t})=>(0,N.jsxs)(`div`,{style:{textAlign:`center`},children:[(0,N.jsx)(`div`,{style:{fontSize:18,fontWeight:500},children:t}),(0,N.jsx)(`div`,{style:{fontSize:11,color:`var(--color-text-tertiary)`},children:e})]},e))})]})}function vr(){let[e,t]=(0,x.useState)(cr[0]),[n,r]=(0,x.useState)(`All`),[i,a]=(0,x.useState)(``);return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100vh`,fontFamily:`var(--font-sans, system-ui, sans-serif)`,background:`var(--color-background-tertiary, #152060)`,color:`var(--color-text-primary)`},children:[(0,N.jsx)(_r,{}),(0,N.jsxs)(`div`,{style:{display:`flex`,flex:1,overflow:`hidden`},children:[(0,N.jsx)(`div`,{style:{width:320,minWidth:260,padding:`20px 20px`,borderRight:`0.5px solid var(--color-border-tertiary)`,background:`var(--color-background-primary)`,overflowY:`auto`},children:(0,N.jsx)(gr,{recipes:cr,selected:e,onSelect:t,category:n,setCategory:r,search:i,setSearch:a})}),(0,N.jsx)(`div`,{style:{flex:1,overflowY:`auto`,padding:`24px 28px`},children:e?(0,N.jsx)(hr,{recipe:e}):(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,padding:40,textAlign:`center`},children:`Select a recipe to get started`})})]})]})}var yr=[{id:`swarm-handoff`,category:`Swarm`,title:`Agent Handoffs with Swarm`,difficulty:`Beginner`,time:`~20 min`,description:`Use OpenAI Swarm-style handoffs to transfer control between specialized agents mid-conversation. Each agent owns a domain and passes the baton when out of scope.`,tags:[`swarm`,`handoff`,`routing`],steps:[{label:`Define Agents`,icon:`🤖`,detail:`Create agents with a name, system prompt, and tool list. Each agent is a specialist — keep system prompts tightly scoped to one domain (billing, support, sales).`},{label:`Handoff Tool`,icon:`🔀`,detail:`Give each agent a transfer_to_X tool for every agent it may hand off to. The tool returns the target agent object — the loop detects this and switches context.`},{label:`Run Loop`,icon:`🔄`,detail:`The orchestration loop calls the active agent, checks if a tool result is an Agent object, and if so switches the active agent for the next turn.`},{label:`Preserve History`,icon:`📚`,detail:`Pass the full conversation history to every agent on every turn. Agents can read what happened before the handoff to avoid asking the user to repeat themselves.`},{label:`Context Variables`,icon:`📦`,detail:`Maintain a shared context_variables dict passed to every tool call. Agents read and write to it — e.g. storing account_id so downstream agents don't re-fetch it.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log("Final answer:", result.text);`},{id:`langgraph-state`,category:`LangGraph`,title:`Stateful Graph with LangGraph`,difficulty:`Intermediate`,time:`~35 min`,description:`Model your agent workflow as a directed graph where nodes are functions and edges are conditional transitions. State flows through the graph, accumulating results at each node.`,tags:[`LangGraph`,`state graph`,`conditional edges`],steps:[{label:`Define State`,icon:`📋`,detail:`Declare a TypeScript interface (or Zod schema) for the shared state object. Every node reads from and writes to this state — it's the single source of truth across the graph.`},{label:`Create Nodes`,icon:`🔵`,detail:`Each node is an async function that receives the current state and returns a partial state update. Nodes should do one thing: call an LLM, invoke a tool, or make a decision.`},{label:`Add Edges`,icon:`➡️`,detail:`Static edges always proceed to the next node. Conditional edges call a router function that inspects state and returns the name of the next node — this is how branching logic works.`},{label:`Checkpointing`,icon:`💾`,detail:`Attach a checkpointer (e.g. MemorySaver or SqliteSaver) to persist state after each node. This enables pause/resume, time-travel debugging, and human-in-the-loop interrupts.`},{label:`Compile & Run`,icon:`▶️`,detail:`Call graph.compile() to validate the graph structure (no orphan nodes, no missing edges). Then invoke with an initial state and a thread config for session isolation.`}],code:`import { Anthropic } from "@anthropic-ai/sdk";
...
console.log("Iterations:", result.iterations);`},{id:`crewai-roles`,category:`CrewAI`,title:`Role-Based Crews`,difficulty:`Intermediate`,time:`~30 min`,description:`Assemble a crew of role-playing agents, each with a goal and backstory. Tasks flow sequentially or in parallel through the crew, with each agent contributing its specialty.`,tags:[`CrewAI`,`roles`,`tasks`,`process`],steps:[{label:`Define Roles`,icon:`🎭`,detail:`Create Agent objects with role, goal, and backstory. The backstory shapes how the LLM interprets its persona — be specific and concrete, not generic.`},{label:`Assign Tools`,icon:`🔧`,detail:`Give each agent the tools it needs for its role. Agents only call tools relevant to their specialty — a researcher gets search, a writer gets none.`},{label:`Create Tasks`,icon:`📋`,detail:`Tasks have a description, expected_output, and an assigned agent. The expected_output guides what format the agent should return — be explicit.`},{label:`Pick a Process`,icon:`⚙️`,detail:`Process.sequential runs tasks one after another, passing each output as context to the next. Process.hierarchical adds a manager agent that delegates and reviews.`},{label:`Kick Off Crew`,icon:`🚀`,detail:`Call crew.kickoff(inputs={...}) with any template variables used in task descriptions. Returns the final task's output as a string.`}],code:`from crewai import Agent, Task, Crew, Process
...
print(result)`},{id:`autogen-conversation`,category:`AutoGen`,title:`Conversable Agents with AutoGen`,difficulty:`Intermediate`,time:`~25 min`,description:`AutoGen's ConversableAgent lets agents chat with each other autonomously. A UserProxyAgent executes code; an AssistantAgent writes it. Together they solve tasks iteratively.`,tags:[`AutoGen`,`conversable`,`code execution`],steps:[{label:`AssistantAgent`,icon:`✍️`,detail:`The AssistantAgent is backed by an LLM and writes code, plans, and responses. Configure it with a system message and llm_config pointing to your model.`},{label:`UserProxyAgent`,icon:`⚙️`,detail:`The UserProxyAgent represents the human side and can execute code locally. Set human_input_mode to NEVER for full autonomy, or TERMINATE to stop on a keyword.`},{label:`Code Execution`,icon:`🖥️`,detail:`UserProxyAgent extracts code blocks from AssistantAgent messages and runs them in a Docker sandbox or local subprocess. Results feed back into the conversation.`},{label:`Termination`,icon:`🛑`,detail:`Define is_termination_msg to stop the conversation when the assistant signals completion (e.g. returns 'TERMINATE'). Without this, conversations loop indefinitely.`},{label:`GroupChat`,icon:`👥`,detail:`For 3+ agents, use GroupChat + GroupChatManager. The manager decides which agent speaks next, enabling round-robin or LLM-selected speaker order.`}],code:`import autogen
from anthropic import Anthropic
...
)`},{id:`custom-orchestrator`,category:`Custom`,title:`Build Your Own Orchestrator`,difficulty:`Advanced`,time:`~50 min`,description:`Skip the frameworks and build a lightweight multi-agent orchestrator from scratch. Full control over routing, state, retries, and observability.`,tags:[`custom`,`orchestration`,`from scratch`],steps:[{label:`Agent Abstraction`,icon:`🤖`,detail:`Define a minimal Agent class: name, system prompt, tools, and an async run(messages) method. Keep it thin — the orchestrator handles routing, not the agent.`},{label:`Message Bus`,icon:`📨`,detail:`Use a shared message list as the bus. Each agent appends its output as {role: assistant, name: agent_name, content: ...}. Downstream agents see the full history.`},{label:`Router`,icon:`🗺️`,detail:`A router function inspects the last message and returns the name of the next agent. Can be rule-based (keyword match), LLM-based (ask Claude who should answer), or hybrid.`},{label:`Retry & Timeout`,icon:`🔁`,detail:`Wrap each agent call in a retry loop with exponential backoff. Set per-agent timeouts so a slow agent doesn't block the entire pipeline.`},{label:`Observability`,icon:`📊`,detail:`Emit structured logs for every agent invocation: agent name, token usage, latency, tool calls, and routing decisions. This is what makes multi-agent systems debuggable.`},{label:`Human-in-the-Loop`,icon:`👤`,detail:`Before routing to high-stakes agents (e.g. one that executes code or sends emails), pause and require human approval. Store pending approvals in a queue the UI polls.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(answer);`},{id:`supervisor-pattern`,category:`Custom`,title:`Supervisor Pattern`,difficulty:`Advanced`,time:`~40 min`,description:`A supervisor agent evaluates worker agent outputs, requests revisions, and gates progress — ensuring quality before results move to the next stage.`,tags:[`supervisor`,`quality gate`,`revision loop`],steps:[{label:`Worker Agent`,icon:`👷`,detail:`Worker agents produce draft outputs: text, code, data, or plans. They should not self-evaluate — that's the supervisor's job. Keep workers focused on production.`},{label:`Supervisor Agent`,icon:`🎓`,detail:`The supervisor receives the worker's output along with the original task. It scores quality and returns either APPROVE or REVISE with specific feedback.`},{label:`Feedback Loop`,icon:`🔄`,detail:`If the supervisor returns REVISE, append the feedback as a new user message and re-invoke the worker. The worker sees its own prior output and the critique.`},{label:`Max Rounds`,icon:`🔢`,detail:`Cap the revision loop at N rounds (typically 3). If the supervisor still rejects after max rounds, escalate to a human or return the best attempt with a warning.`},{label:`Structured Eval`,icon:`📊`,detail:`Ask the supervisor to return a JSON scorecard: {score, approved, feedback, criteria}. Structured output makes it easy to log quality metrics over time.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(\`\\nApproved: \${result.approved} | Rounds: \${result.rounds}\`);`},{id:`parallelization`,category:`Patterns`,title:`Parallel Agent Fan-Out`,difficulty:`Intermediate`,time:`~25 min`,description:`Split a task into independent subtasks, run specialist agents on each in parallel, then aggregate. Dramatically reduces latency for decomposable workloads.`,tags:[`parallelism`,`fan-out`,`aggregation`],steps:[{label:`Decompose Task`,icon:`✂️`,detail:`Use an LLM to split the main task into N independent subtasks. Independence is key — subtasks must not depend on each other's outputs for parallel execution to work.`},{label:`Spawn Workers`,icon:`⚡`,detail:`Map each subtask to a Promise and run all with Promise.all. Each worker is a separate LLM call with its own system prompt and tools — true parallel execution.`},{label:`Error Isolation`,icon:`🛡️`,detail:`Use Promise.allSettled instead of Promise.all to prevent one failed worker from cancelling the others. Inspect each result's status independently.`},{label:`Aggregate Results`,icon:`🔗`,detail:`Collect all fulfilled worker outputs into a single context and pass to a synthesis agent. The synthesis agent's job is to merge, deduplicate, and resolve conflicts.`},{label:`Token Budget`,icon:`💰`,detail:`Parallel workers multiply your token usage. Set per-worker max_tokens budgets to avoid runaway costs. Consider using a smaller model (e.g. claude-haiku-4-5) for subtasks.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log("\\n── Final Answer ──────────────────────────────────");
console.log(answer);`}],br=[`All`,`Swarm`,`LangGraph`,`CrewAI`,`AutoGen`,`Custom`,`Patterns`],xr={Beginner:`#0F6E56`,Intermediate:`#185FA5`,Advanced:`#993C1D`},Sr={Beginner:`#E1F5EE`,Intermediate:`#E6F1FB`,Advanced:`#FAECE7`};function Cr({steps:e}){let[t,n]=(0,x.useState)(null);return(0,N.jsxs)(`div`,{style:{marginTop:16},children:[(0,N.jsx)(`div`,{style:{display:`flex`,flexWrap:`wrap`,gap:8,alignItems:`center`},children:e.map((r,i)=>(0,N.jsxs)(`div`,{style:{display:`flex`,alignItems:`center`,gap:6},children:[(0,N.jsxs)(`button`,{onClick:()=>n(t===i?null:i),style:{display:`flex`,alignItems:`center`,gap:6,padding:`6px 12px`,borderRadius:20,border:t===i?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:t===i?`#E6F1FB`:`var(--color-background-primary)`,color:t===i?`#185FA5`:`var(--color-text-primary)`,cursor:`pointer`,fontSize:13,fontWeight:t===i?500:400,transition:`all 0.15s`},children:[(0,N.jsx)(`span`,{children:r.icon}),(0,N.jsx)(`span`,{children:r.label})]}),i<e.length-1&&(0,N.jsx)(`span`,{style:{color:`var(--color-text-tertiary)`,fontSize:12},children:`→`})]},i))}),t!==null&&(0,N.jsxs)(`div`,{style:{marginTop:10,padding:`10px 14px`,borderRadius:8,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.6},children:[(0,N.jsxs)(`span`,{style:{fontWeight:500,color:`var(--color-text-primary)`},children:[e[t].label,`: `]}),e[t].detail]})]})}function wr({code:e}){let[t,n]=(0,x.useState)(!1);return(0,N.jsxs)(`div`,{style:{position:`relative`,marginTop:16},children:[(0,N.jsx)(`button`,{onClick:()=>{navigator.clipboard.writeText(e),n(!0),setTimeout(()=>n(!1),1800)},style:{position:`absolute`,top:8,right:8,padding:`4px 10px`,borderRadius:6,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,cursor:`pointer`,fontSize:12,color:`var(--color-text-secondary)`,zIndex:1},children:t?`✓ Copied`:`Copy`}),(0,N.jsx)(`pre`,{style:{margin:0,padding:`14px 16px`,borderRadius:10,overflowX:`auto`,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:12,lineHeight:1.65,fontFamily:`var(--font-mono)`,color:`var(--color-text-primary)`,whiteSpace:`pre`},children:(0,N.jsx)(`code`,{children:e})})]})}function Tr({recipe:e,onSelect:t,selected:n}){return(0,N.jsxs)(`div`,{onClick:()=>t(e),style:{padding:`16px 18px`,borderRadius:12,cursor:`pointer`,border:n?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:n?`#061320`:`var(--color-background-primary)`,transition:`all 0.15s`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:6},children:[(0,N.jsx)(`span`,{style:{fontSize:13,color:`var(--color-text-secondary)`,fontWeight:400},children:e.category}),(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,fontWeight:500,background:Sr[e.difficulty],color:xr[e.difficulty]},children:e.difficulty})]}),(0,N.jsx)(`div`,{style:{fontWeight:500,fontSize:15,marginBottom:4,color:`var(--color-text-primary)`},children:e.title}),(0,N.jsx)(`div`,{style:{fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.5},children:e.description}),(0,N.jsx)(`div`,{style:{marginTop:10,display:`flex`,gap:6,flexWrap:`wrap`},children:e.tags.map(e=>(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,background:`var(--color-background-tertiary)`,color:`var(--color-text-secondary)`,border:`0.5px solid var(--color-border-tertiary)`},children:e},e))})]})}function Er({recipe:e}){let[t,n]=(0,x.useState)(`steps`);return(0,N.jsxs)(`div`,{style:{padding:`24px`,borderRadius:14,background:`var(--color-background-primary)`,border:`0.5px solid var(--color-border-tertiary)`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:4},children:[(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:e.category}),(0,N.jsx)(`h2`,{style:{margin:`4px 0 6px`,fontSize:22,fontWeight:500},children:e.title})]}),(0,N.jsxs)(`div`,{style:{display:`flex`,gap:8,alignItems:`center`,paddingTop:4},children:[(0,N.jsx)(`span`,{style:{fontSize:12,padding:`3px 10px`,borderRadius:20,fontWeight:500,background:Sr[e.difficulty],color:xr[e.difficulty]},children:e.difficulty}),(0,N.jsxs)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:[`⏱ `,e.time]})]})]}),(0,N.jsx)(`p`,{style:{margin:`0 0 20px`,color:`var(--color-text-secondary)`,fontSize:14,lineHeight:1.6},children:e.description}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:4,marginBottom:18,borderBottom:`0.5px solid var(--color-border-tertiary)`,paddingBottom:0},children:[`steps`,`code`].map(e=>(0,N.jsx)(`button`,{onClick:()=>n(e),style:{padding:`8px 16px`,border:`none`,background:`none`,cursor:`pointer`,fontSize:14,fontWeight:t===e?500:400,color:t===e?`var(--color-text-primary)`:`var(--color-text-secondary)`,borderBottom:t===e?`2px solid #185FA5`:`2px solid transparent`,marginBottom:-1,transition:`all 0.12s`},children:e===`steps`?`Pipeline Steps`:`Code`},e))}),t===`steps`&&(0,N.jsx)(Cr,{steps:e.steps}),t===`code`&&(0,N.jsx)(wr,{code:e.code})]})}function Dr({recipes:e,selected:t,onSelect:n,category:r,setCategory:i,search:a,setSearch:o}){let s=e.filter(e=>{let t=r===`All`||e.category===r,n=e.title.toLowerCase().includes(a.toLowerCase())||e.tags.some(e=>e.toLowerCase().includes(a.toLowerCase()));return t&&n});return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100%`,gap:0},children:[(0,N.jsx)(`div`,{style:{padding:`0 0 16px`},children:(0,N.jsx)(`input`,{type:`text`,placeholder:`Search recipes…`,value:a,onChange:e=>o(e.target.value),style:{width:`100%`,boxSizing:`border-box`,padding:`8px 12px`,borderRadius:8,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,color:`var(--color-text-primary)`,fontSize:13}})}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:6,flexWrap:`wrap`,marginBottom:16},children:br.map(e=>(0,N.jsx)(`button`,{onClick:()=>i(e),style:{padding:`4px 12px`,borderRadius:20,fontSize:12,cursor:`pointer`,border:r===e?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:r===e?`#E6F1FB`:`var(--color-background-primary)`,color:r===e?`#185FA5`:`var(--color-text-secondary)`,fontWeight:r===e?500:400},children:e},e))}),(0,N.jsx)(`div`,{style:{display:`flex`,flexDirection:`column`,gap:10,overflowY:`auto`,flex:1},children:s.length===0?(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,fontSize:13,padding:`12px 0`},children:`No recipes found.`}):s.map(e=>(0,N.jsx)(Tr,{recipe:e,onSelect:n,selected:t?.id===e.id},e.id))})]})}function Or(){return(0,N.jsxs)(`div`,{style:{padding:`20px 32px 16px`,borderBottom:`0.5px solid var(--color-border-tertiary)`,display:`flex`,alignItems:`center`,gap:16},children:[(0,N.jsx)(`div`,{style:{width:40,height:40,borderRadius:10,background:`#E1F5EE`,display:`flex`,alignItems:`center`,justifyContent:`center`,fontSize:20},children:`🧩`}),(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`h1`,{style:{margin:0,fontSize:20,fontWeight:500,letterSpacing:`-0.3px`},children:`Multi-Agent Frameworks`}),(0,N.jsx)(`p`,{style:{margin:0,fontSize:13,color:`var(--color-text-secondary)`},children:`End-to-end recipes for Swarm, LangGraph, CrewAI, AutoGen & custom orchestration`})]}),(0,N.jsx)(`div`,{style:{marginLeft:`auto`,display:`flex`,gap:20},children:[{label:`Recipes`,value:yr.length},{label:`Frameworks`,value:br.length-1}].map(({label:e,value:t})=>(0,N.jsxs)(`div`,{style:{textAlign:`center`},children:[(0,N.jsx)(`div`,{style:{fontSize:18,fontWeight:500},children:t}),(0,N.jsx)(`div`,{style:{fontSize:11,color:`var(--color-text-tertiary)`},children:e})]},e))})]})}function kr(){let[e,t]=(0,x.useState)(yr[0]),[n,r]=(0,x.useState)(`All`),[i,a]=(0,x.useState)(``);return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100vh`,fontFamily:`var(--font-sans, system-ui, sans-serif)`,background:`var(--color-background-tertiary, #152060)`,color:`var(--color-text-primary)`},children:[(0,N.jsx)(Or,{}),(0,N.jsxs)(`div`,{style:{display:`flex`,flex:1,overflow:`hidden`},children:[(0,N.jsx)(`div`,{style:{width:320,minWidth:260,padding:`20px 20px`,borderRight:`0.5px solid var(--color-border-tertiary)`,background:`var(--color-background-primary)`,overflowY:`auto`},children:(0,N.jsx)(Dr,{recipes:yr,selected:e,onSelect:t,category:n,setCategory:r,search:i,setSearch:a})}),(0,N.jsx)(`div`,{style:{flex:1,overflowY:`auto`,padding:`24px 28px`},children:e?(0,N.jsx)(Er,{recipe:e}):(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,padding:40,textAlign:`center`},children:`Select a recipe to get started`})})]})]})}var Ar=[{id:`mcp-server-basic`,category:`Foundations`,title:`Your First MCP Server`,difficulty:`Beginner`,time:`~15 min`,description:`Scaffold a minimal MCP server that exposes one tool. The entry point for understanding how hosts, clients, and servers connect.`,tags:[`server`,`stdio`,`tools`],steps:[{label:`Install SDK`,icon:`📦`,detail:`npm install @modelcontextprotocol/sdk — the official TypeScript SDK handles protocol framing, schema validation, and transport.`},{label:`Create Server`,icon:`🖥️`,detail:`Instantiate a Server with a name and version. This becomes the identity your MCP host (e.g. Claude Desktop) sees in its registry.`},{label:`Register Tools`,icon:`🔧`,detail:`Call server.tool(name, description, zodSchema, handler). The SDK auto-generates the JSON Schema from Zod and routes incoming calls to your handler.`},{label:`Connect Transport`,icon:`🔌`,detail:`Wrap the server in a StdioServerTransport — the server reads JSON-RPC messages from stdin and writes responses to stdout.`},{label:`Run & Test`,icon:`🧪`,detail:`Point Claude Desktop or the MCP Inspector at your server binary. Use the Inspector's UI to call tools and inspect raw protocol messages.`}],code:`import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
...
//     }
//   }
// }`},{id:`mcp-resources`,category:`Foundations`,title:`Exposing Resources`,difficulty:`Beginner`,time:`~20 min`,description:`Resources let your server expose readable content — files, DB rows, API responses — that the host can embed directly into the LLM context.`,tags:[`resources`,`context`,`URIs`],steps:[{label:`Define URI Scheme`,icon:`🔗`,detail:`Choose a URI scheme for your resource namespace, e.g. file://, db://, or myapp://. URIs uniquely identify each resource.`},{label:`List Resources`,icon:`📋`,detail:`Implement the resources/list handler to return an array of {uri, name, description, mimeType}. The host calls this to discover what's available.`},{label:`Read Handler`,icon:`📖`,detail:`Implement resources/read: given a URI, return {contents: [{uri, mimeType, text|blob}]}. Text is passed as-is; blobs are base64-encoded.`},{label:`Resource Templates`,icon:`🗂️`,detail:`Use URI templates (RFC 6570) like file:///{path} for dynamic resources. The host fills in parameters before calling read.`},{label:`Change Notifications`,icon:`🔔`,detail:`Emit notifications/resources/updated when a resource changes. Hosts that support subscriptions will re-fetch and update their context.`}],code:`import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
...
const transport = new StdioServerTransport();
await server.connect(transport);`},{id:`mcp-prompts`,category:`Foundations`,title:`Reusable Prompts`,difficulty:`Beginner`,time:`~15 min`,description:`Register prompt templates on your server so any MCP host can invoke them by name with typed arguments — a shared prompt library across clients.`,tags:[`prompts`,`templates`,`reusability`],steps:[{label:`Define Prompt`,icon:`📝`,detail:`Call server.prompt(name, description, argsSchema, handler). Args are typed via Zod; descriptions help the host surface them in UI.`},{label:`Build Messages`,icon:`💬`,detail:`Return an array of {role, content} objects. Prompts can mix user and assistant turns to set up a conversation scaffold.`},{label:`Embed Resources`,icon:`🔗`,detail:`Reference resource URIs inside prompt content using {type: 'resource', resource: {uri}}. The host resolves and embeds the resource at runtime.`},{label:`List & Get`,icon:`📋`,detail:`The host calls prompts/list to discover prompts and prompts/get with args to render them. Your handler receives the filled-in arguments.`},{label:`Prompt Chaining`,icon:`🔄`,detail:`Compose complex prompt chains by referencing other prompts as resources, enabling modular, reusable conversation patterns.`}],code:`import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
...
const transport = new StdioServerTransport();
await server.connect(transport);`},{id:`mcp-client`,category:`Client`,title:`MCP Client Integration`,difficulty:`Intermediate`,time:`~30 min`,description:`Build an MCP client that connects to a server, discovers its capabilities, and passes tools to Claude — the bridge between your server and the LLM.`,tags:[`client`,`discovery`,`Claude API`],steps:[{label:`Launch Server`,icon:`🚀`,detail:`Spawn the MCP server as a child process. The client manages its lifecycle, restarting on crash if needed.`},{label:`Create Client`,icon:`🔌`,detail:`Instantiate a Client with name/version and connect via StdioClientTransport pointed at the server process.`},{label:`Discover Tools`,icon:`🔍`,detail:`Call client.listTools() to get the full tool catalog: name, description, and inputSchema for each tool the server exposes.`},{label:`Convert Schemas`,icon:`🔄`,detail:`Map MCP tool schemas to Anthropic tool format — both use JSON Schema, so conversion is typically a direct pass-through with minor reshaping.`},{label:`Claude Tool Loop`,icon:`🤖`,detail:`Pass converted tools to claude.messages.create. When Claude returns tool_use blocks, route them back through client.callTool() on your MCP server.`},{label:`Return Results`,icon:`📤`,detail:`Feed tool_result blocks back to Claude and continue the loop until stop_reason === 'end_turn'.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(answer);`},{id:`mcp-auth`,category:`Production`,title:`OAuth & Remote Servers`,difficulty:`Advanced`,time:`~60 min`,description:`Serve MCP over HTTP with Server-Sent Events and protect it with OAuth 2.1. Enables multi-tenant, cloud-hosted MCP servers.`,tags:[`OAuth`,`HTTP`,`SSE`,`multi-tenant`],steps:[{label:`HTTP + SSE Transport`,icon:`🌐`,detail:`Replace stdio with StreamableHTTPServerTransport. The client POSTs JSON-RPC to /mcp and receives streamed responses via SSE.`},{label:`OAuth 2.1 Flow`,icon:`🔐`,detail:`Implement the MCP OAuth spec: discovery endpoint at /.well-known/oauth-authorization-server, authorization, token, and revocation endpoints.`},{label:`Token Validation`,icon:`✅`,detail:`Validate Bearer tokens on every request in middleware. Reject with 401 if missing, expired, or tampered. Attach the user identity to the request context.`},{label:`Dynamic Registration`,icon:`📋`,detail:`Support RFC 7591 dynamic client registration at /register so MCP hosts can self-register without manual setup.`},{label:`Session Management`,icon:`🗝️`,detail:`Maintain per-session server instances keyed by Mcp-Session-Id header. Clean up on session close or timeout to avoid resource leaks.`},{label:`PKCE + HTTPS`,icon:`🛡️`,detail:`Require PKCE for all authorization code flows. Enforce HTTPS in production — the MCP spec mandates this for remote servers.`}],code:`import express from "express";
...
app.listen(3000, () => console.log("MCP server listening on :3000"));`},{id:`mcp-sampling`,category:`Advanced`,title:`Sampling & Roots`,difficulty:`Advanced`,time:`~40 min`,description:`Use MCP's sampling primitive to let your server call the LLM through the host, and roots to understand the client's file system workspace.`,tags:[`sampling`,`roots`,`LLM-in-server`],steps:[{label:`Declare Capability`,icon:`📢`,detail:`Advertise {sampling: {}} in your server capabilities. Hosts that support sampling will enable the createMessage API on your server instance.`},{label:`Call createMessage`,icon:`💬`,detail:`Inside a tool handler, call server.server.createMessage({messages, maxTokens}). The host proxies this to its LLM — your server never needs its own API key.`},{label:`Human-in-the-Loop`,icon:`👤`,detail:`The host may show the user a confirmation dialog before forwarding the sampling request. Design your tool to handle both approval and rejection gracefully.`},{label:`List Roots`,icon:`📁`,detail:`Call client.listRoots() (from server context) to discover the directories the user has opened in their IDE or file explorer — your workspace context.`},{label:`Watch Root Changes`,icon:`🔔`,detail:`Subscribe to roots/list_changed notifications to stay in sync when the user opens or closes project folders during a session.`},{label:`Scoped Access`,icon:`🔒`,detail:`Restrict all file operations to the declared roots. Never traverse outside them — the roots define the security boundary the user has explicitly granted.`}],code:`import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
...
const transport = new StdioServerTransport();
await server.connect(transport);`},{id:`mcp-testing`,category:`Production`,title:`Testing & Debugging`,difficulty:`Intermediate`,time:`~30 min`,description:`Test MCP servers with the official Inspector, write automated unit tests for tools, and debug protocol messages without a full host.`,tags:[`testing`,`inspector`,`debugging`],steps:[{label:`MCP Inspector`,icon:`🔬`,detail:`Run npx @modelcontextprotocol/inspector node ./server.js to open a browser UI. Call tools, browse resources, and inspect raw JSON-RPC messages interactively.`},{label:`In-Memory Transport`,icon:`🧪`,detail:`Use InMemoryTransport for unit tests — no child processes needed. Create linked client/server pairs and test tool logic directly in your test runner.`},{label:`Test Tool Schemas`,icon:`📋`,detail:`Verify your tool input schemas reject invalid inputs. The SDK validates against JSON Schema before calling your handler — test that validation too.`},{label:`Mock Tool Results`,icon:`🎭`,detail:`In integration tests, mock external services (APIs, DBs) at the tool handler boundary. Keep MCP protocol behavior real; fake only the I/O side effects.`},{label:`Protocol Logging`,icon:`📝`,detail:`Set MCP_LOG_LEVEL=debug to stream all JSON-RPC traffic to stderr. Pipe to jq for pretty-printing: node server.js 2>&1 | grep MCP | jq .`}],code:`import { describe, it, expect, beforeEach, afterEach } from "vitest";
import { Client } from "@modelcontextprotocol/sdk/client/index.js";
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
...
// Run: npx vitest run
// Inspector: npx @modelcontextprotocol/inspector node ./server.js`}],jr=[`All`,`Foundations`,`Client`,`Production`,`Advanced`],Mr={Beginner:`#0F6E56`,Intermediate:`#185FA5`,Advanced:`#993C1D`},Nr={Beginner:`#E1F5EE`,Intermediate:`#E6F1FB`,Advanced:`#FAECE7`};function Pr({steps:e}){let[t,n]=(0,x.useState)(null);return(0,N.jsxs)(`div`,{style:{marginTop:16},children:[(0,N.jsx)(`div`,{style:{display:`flex`,flexWrap:`wrap`,gap:8,alignItems:`center`},children:e.map((r,i)=>(0,N.jsxs)(`div`,{style:{display:`flex`,alignItems:`center`,gap:6},children:[(0,N.jsxs)(`button`,{onClick:()=>n(t===i?null:i),style:{display:`flex`,alignItems:`center`,gap:6,padding:`6px 12px`,borderRadius:20,border:t===i?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:t===i?`#E6F1FB`:`var(--color-background-primary)`,color:t===i?`#185FA5`:`var(--color-text-primary)`,cursor:`pointer`,fontSize:13,fontWeight:t===i?500:400,transition:`all 0.15s`},children:[(0,N.jsx)(`span`,{children:r.icon}),(0,N.jsx)(`span`,{children:r.label})]}),i<e.length-1&&(0,N.jsx)(`span`,{style:{color:`var(--color-text-tertiary)`,fontSize:12},children:`→`})]},i))}),t!==null&&(0,N.jsxs)(`div`,{style:{marginTop:10,padding:`10px 14px`,borderRadius:8,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.6},children:[(0,N.jsxs)(`span`,{style:{fontWeight:500,color:`var(--color-text-primary)`},children:[e[t].label,`: `]}),e[t].detail]})]})}function Fr({code:e}){let[t,n]=(0,x.useState)(!1);return(0,N.jsxs)(`div`,{style:{position:`relative`,marginTop:16},children:[(0,N.jsx)(`button`,{onClick:()=>{navigator.clipboard.writeText(e),n(!0),setTimeout(()=>n(!1),1800)},style:{position:`absolute`,top:8,right:8,padding:`4px 10px`,borderRadius:6,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,cursor:`pointer`,fontSize:12,color:`var(--color-text-secondary)`,zIndex:1},children:t?`✓ Copied`:`Copy`}),(0,N.jsx)(`pre`,{style:{margin:0,padding:`14px 16px`,borderRadius:10,overflowX:`auto`,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:12,lineHeight:1.65,fontFamily:`var(--font-mono)`,color:`var(--color-text-primary)`,whiteSpace:`pre`},children:(0,N.jsx)(`code`,{children:e})})]})}function Ir({recipe:e,onSelect:t,selected:n}){return(0,N.jsxs)(`div`,{onClick:()=>t(e),style:{padding:`16px 18px`,borderRadius:12,cursor:`pointer`,border:n?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:n?`#061320`:`var(--color-background-primary)`,transition:`all 0.15s`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:6},children:[(0,N.jsx)(`span`,{style:{fontSize:13,color:`var(--color-text-secondary)`,fontWeight:400},children:e.category}),(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,fontWeight:500,background:Nr[e.difficulty],color:Mr[e.difficulty]},children:e.difficulty})]}),(0,N.jsx)(`div`,{style:{fontWeight:500,fontSize:15,marginBottom:4,color:`var(--color-text-primary)`},children:e.title}),(0,N.jsx)(`div`,{style:{fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.5},children:e.description}),(0,N.jsx)(`div`,{style:{marginTop:10,display:`flex`,gap:6,flexWrap:`wrap`},children:e.tags.map(e=>(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,background:`var(--color-background-tertiary)`,color:`var(--color-text-secondary)`,border:`0.5px solid var(--color-border-tertiary)`},children:e},e))})]})}function Lr({recipe:e}){let[t,n]=(0,x.useState)(`steps`);return(0,N.jsxs)(`div`,{style:{padding:`24px`,borderRadius:14,background:`var(--color-background-primary)`,border:`0.5px solid var(--color-border-tertiary)`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:4},children:[(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:e.category}),(0,N.jsx)(`h2`,{style:{margin:`4px 0 6px`,fontSize:22,fontWeight:500},children:e.title})]}),(0,N.jsxs)(`div`,{style:{display:`flex`,gap:8,alignItems:`center`,paddingTop:4},children:[(0,N.jsx)(`span`,{style:{fontSize:12,padding:`3px 10px`,borderRadius:20,fontWeight:500,background:Nr[e.difficulty],color:Mr[e.difficulty]},children:e.difficulty}),(0,N.jsxs)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:[`⏱ `,e.time]})]})]}),(0,N.jsx)(`p`,{style:{margin:`0 0 20px`,color:`var(--color-text-secondary)`,fontSize:14,lineHeight:1.6},children:e.description}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:4,marginBottom:18,borderBottom:`0.5px solid var(--color-border-tertiary)`,paddingBottom:0},children:[`steps`,`code`].map(e=>(0,N.jsx)(`button`,{onClick:()=>n(e),style:{padding:`8px 16px`,border:`none`,background:`none`,cursor:`pointer`,fontSize:14,fontWeight:t===e?500:400,color:t===e?`var(--color-text-primary)`:`var(--color-text-secondary)`,borderBottom:t===e?`2px solid #185FA5`:`2px solid transparent`,marginBottom:-1,transition:`all 0.12s`},children:e===`steps`?`Pipeline Steps`:`Code`},e))}),t===`steps`&&(0,N.jsx)(Pr,{steps:e.steps}),t===`code`&&(0,N.jsx)(Fr,{code:e.code})]})}function Rr({recipes:e,selected:t,onSelect:n,category:r,setCategory:i,search:a,setSearch:o}){let s=e.filter(e=>{let t=r===`All`||e.category===r,n=e.title.toLowerCase().includes(a.toLowerCase())||e.tags.some(e=>e.toLowerCase().includes(a.toLowerCase()));return t&&n});return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100%`,gap:0},children:[(0,N.jsx)(`div`,{style:{padding:`0 0 16px`},children:(0,N.jsx)(`input`,{type:`text`,placeholder:`Search recipes…`,value:a,onChange:e=>o(e.target.value),style:{width:`100%`,boxSizing:`border-box`,padding:`8px 12px`,borderRadius:8,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,color:`var(--color-text-primary)`,fontSize:13}})}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:6,flexWrap:`wrap`,marginBottom:16},children:jr.map(e=>(0,N.jsx)(`button`,{onClick:()=>i(e),style:{padding:`4px 12px`,borderRadius:20,fontSize:12,cursor:`pointer`,border:r===e?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:r===e?`#E6F1FB`:`var(--color-background-primary)`,color:r===e?`#185FA5`:`var(--color-text-secondary)`,fontWeight:r===e?500:400},children:e},e))}),(0,N.jsx)(`div`,{style:{display:`flex`,flexDirection:`column`,gap:10,overflowY:`auto`,flex:1},children:s.length===0?(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,fontSize:13,padding:`12px 0`},children:`No recipes found.`}):s.map(e=>(0,N.jsx)(Ir,{recipe:e,onSelect:n,selected:t?.id===e.id},e.id))})]})}function zr(){return(0,N.jsxs)(`div`,{style:{padding:`20px 32px 16px`,borderBottom:`0.5px solid var(--color-border-tertiary)`,display:`flex`,alignItems:`center`,gap:16},children:[(0,N.jsx)(`div`,{style:{width:40,height:40,borderRadius:10,background:`#FAECE7`,display:`flex`,alignItems:`center`,justifyContent:`center`,fontSize:20},children:`🔌`}),(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`h1`,{style:{margin:0,fontSize:20,fontWeight:500,letterSpacing:`-0.3px`},children:`MCP Cookbook`}),(0,N.jsx)(`p`,{style:{margin:0,fontSize:13,color:`var(--color-text-secondary)`},children:`End-to-end Model Context Protocol recipes`})]}),(0,N.jsx)(`div`,{style:{marginLeft:`auto`,display:`flex`,gap:20},children:[{label:`Recipes`,value:Ar.length},{label:`Patterns`,value:jr.length-1}].map(({label:e,value:t})=>(0,N.jsxs)(`div`,{style:{textAlign:`center`},children:[(0,N.jsx)(`div`,{style:{fontSize:18,fontWeight:500},children:t}),(0,N.jsx)(`div`,{style:{fontSize:11,color:`var(--color-text-tertiary)`},children:e})]},e))})]})}function Br(){let[e,t]=(0,x.useState)(Ar[0]),[n,r]=(0,x.useState)(`All`),[i,a]=(0,x.useState)(``);return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100vh`,fontFamily:`var(--font-sans, system-ui, sans-serif)`,background:`var(--color-background-tertiary, #152060)`,color:`var(--color-text-primary)`},children:[(0,N.jsx)(zr,{}),(0,N.jsxs)(`div`,{style:{display:`flex`,flex:1,overflow:`hidden`},children:[(0,N.jsx)(`div`,{style:{width:320,minWidth:260,padding:`20px 20px`,borderRight:`0.5px solid var(--color-border-tertiary)`,background:`var(--color-background-primary)`,overflowY:`auto`},children:(0,N.jsx)(Rr,{recipes:Ar,selected:e,onSelect:t,category:n,setCategory:r,search:i,setSearch:a})}),(0,N.jsx)(`div`,{style:{flex:1,overflowY:`auto`,padding:`24px 28px`},children:e?(0,N.jsx)(Lr,{recipe:e}):(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,padding:40,textAlign:`center`},children:`Select a recipe to get started`})})]})]})}var Vr=[{id:`a2a-agent-card`,category:`Foundations`,title:`Agent Card & Discovery`,difficulty:`Beginner`,time:`~15 min`,description:`Every A2A agent publishes an Agent Card at /.well-known/agent.json — a machine-readable identity document listing capabilities, skills, and endpoints.`,tags:[`agent card`,`discovery`,`well-known`],steps:[{label:`Define Agent Card`,icon:`🪪`,detail:`Create an agent.json with name, description, version, url, and capabilities. This is the agent's public identity — keep descriptions precise so client agents can route tasks correctly.`},{label:`List Skills`,icon:`🎯`,detail:`Each skill has an id, name, description, and input/output modes (text, file, data). Skills are the unit of capability discovery — agents pick skills by matching them to tasks.`},{label:`Declare Auth`,icon:`🔐`,detail:`Specify authentication schemes (none, bearer, oauth2) in the card. Client agents read this before initiating contact to know which credentials to attach.`},{label:`Serve at Well-Known`,icon:`🌐`,detail:`Host the card at GET /.well-known/agent.json with Content-Type: application/json. This URL is the universal discovery entry point in the A2A protocol.`},{label:`Validate Schema`,icon:`✅`,detail:`Validate your card against the official A2A JSON Schema. Malformed cards silently break discovery — schema validation catches issues before deployment.`}],code:`import express from "express";
...
const card = await discoverAgent("https://my-agent.example.com");`},{id:`a2a-task-lifecycle`,category:`Foundations`,title:`Task Lifecycle`,difficulty:`Beginner`,time:`~20 min`,description:`Tasks are the core unit of work in A2A. Learn how tasks are created, transition through states, and return artifacts to the client agent.`,tags:[`tasks`,`state machine`,`artifacts`],steps:[{label:`Send Task`,icon:`📤`,detail:`POST to /tasks/send with a unique taskId, skill id, and message. The taskId is client-generated — use a UUID. The server must be idempotent on duplicate taskIds.`},{label:`submitted → working`,icon:`⚙️`,detail:`The server acknowledges immediately with status: submitted, then transitions to working as it begins processing. Return the full Task object in the response.`},{label:`working → completed`,icon:`✅`,detail:`On success, set status to completed and attach artifacts (text, data, or file parts) to the task. Artifacts are the deliverables the client agent consumes.`},{label:`working → failed`,icon:`❌`,detail:`On error, transition to failed and populate the error field with a code and message. Client agents must handle this state explicitly.`},{label:`Poll or Stream`,icon:`🔄`,detail:`Client agents poll GET /tasks/{id} for status, or subscribe to SSE at /tasks/{id}/subscribe for push updates. Prefer streaming for long-running tasks.`},{label:`canceled state`,icon:`🛑`,detail:`Client agents can abort work with POST /tasks/{id}/cancel. Servers should check for cancellation at yield points and transition to canceled gracefully.`}],code:`import express from "express";
import { randomUUID } from "crypto";
...
app.listen(3000);`},{id:`a2a-streaming`,category:`Streaming`,title:`Streaming with SSE`,difficulty:`Intermediate`,time:`~25 min`,description:`Stream task progress and partial artifacts to the client agent in real time using Server-Sent Events — no polling required.`,tags:[`SSE`,`streaming`,`real-time`],steps:[{label:`tasks/sendSubscribe`,icon:`📡`,detail:`Client sends POST /tasks/sendSubscribe instead of /tasks/send. The connection stays open; the server streams events over it as SSE.`},{label:`Set SSE Headers`,icon:`🔧`,detail:`Set Content-Type: text/event-stream, Cache-Control: no-cache, Connection: keep-alive. Disable response buffering (res.flushHeaders()) so events flow immediately.`},{label:`Emit Status Events`,icon:`📊`,detail:`Send TaskStatusUpdateEvent as the task transitions: submitted → working. Each event is a JSON object with id, status, and timestamp.`},{label:`Stream Artifacts`,icon:`🧩`,detail:`Emit TaskArtifactUpdateEvent with partial artifact chunks as they're produced. Clients can render incrementally — ideal for long text or large data payloads.`},{label:`Final Event`,icon:`🏁`,detail:`Send a final TaskStatusUpdateEvent with status: completed (or failed) and final: true. The client closes the SSE connection on receipt of a final event.`},{label:`Reconnection`,icon:`🔁`,detail:`Set event id fields so clients can reconnect with Last-Event-ID and resume from where they left off without reprocessing earlier chunks.`}],code:`import express from "express";
import { randomUUID } from "crypto";
...
app.listen(3000);`},{id:`a2a-multi-turn`,category:`Streaming`,title:`Multi-Turn Conversations`,difficulty:`Intermediate`,time:`~30 min`,description:`A2A supports stateful back-and-forth: an agent can pause a task in input-required state and wait for the client to supply additional information before continuing.`,tags:[`multi-turn`,`input-required`,`stateful`],steps:[{label:`input-required State`,icon:`⏸️`,detail:`When the server needs clarification, transition the task to input-required and emit a message explaining what's needed. The task is paused — no artifact produced yet.`},{label:`Client Receives Pause`,icon:`📥`,detail:`The client agent receives the input-required status event with the agent's question. It may ask its own LLM, its user, or another agent to supply the answer.`},{label:`tasks/send Again`,icon:`📤`,detail:`Client resumes the task by POSTing to /tasks/send with the same taskId and a new message containing the requested input. The server continues processing.`},{label:`Conversation History`,icon:`📚`,detail:`The server appends each new message to task.messages. This full history is available to the processing logic and can be passed to the LLM as conversation context.`},{label:`Loop Until Done`,icon:`🔄`,detail:`The pattern repeats: working → input-required → working → ... → completed. There's no limit on turns; design your agent to detect when it has enough information.`}],code:`import express from "express";
...
app.listen(3000);`},{id:`a2a-agent-orchestration`,category:`Multi-Agent`,title:`Agent-to-Agent Delegation`,difficulty:`Advanced`,time:`~45 min`,description:`An orchestrator agent discovers specialist agents via their Agent Cards and delegates subtasks to them over the A2A protocol — no shared codebase required.`,tags:[`orchestration`,`delegation`,`discovery`],steps:[{label:`Discover Agents`,icon:`🔍`,detail:`Fetch /.well-known/agent.json from each known agent URL. Parse skills and capabilities to build a registry of what each agent can do.`},{label:`Route by Skill`,icon:`🗺️`,detail:`Match the subtask description against skill tags and descriptions using keyword or semantic matching. Pick the best-fit agent for each piece of work.`},{label:`Delegate via A2A`,icon:`📤`,detail:`POST /tasks/send to the chosen agent with a well-formed message. Include the parent taskId in metadata so you can correlate results later.`},{label:`Parallel Dispatch`,icon:`⚡`,detail:`Fan out independent subtasks to multiple agents simultaneously with Promise.all. Each agent runs in its own process — true parallelism without shared state.`},{label:`Collect Results`,icon:`📥`,detail:`Poll or stream each delegated task. Aggregate artifacts from all sub-tasks into the orchestrator's context for final synthesis.`},{label:`Synthesize`,icon:`✨`,detail:`Pass all collected artifacts to the orchestrator's LLM to produce a unified final answer, resolving any conflicts or gaps between specialist outputs.`}],code:`import Anthropic from "@anthropic-ai/sdk";
...
console.log(result);`},{id:`a2a-push-notifications`,category:`Production`,title:`Push Notifications`,difficulty:`Intermediate`,time:`~30 min`,description:`For fire-and-forget tasks, agents register a webhook so the server can push completion events without the client holding an open connection.`,tags:[`webhooks`,`push`,`async`],steps:[{label:`Register Webhook`,icon:`🔗`,detail:`Include a pushNotification object in the task request with url and optional authentication headers. The server stores this alongside the task.`},{label:`Validate URL`,icon:`✅`,detail:`Before accepting the task, send a GET challenge request to the webhook URL to verify it's reachable and under the client's control. Reject unverifiable URLs.`},{label:`Process Task`,icon:`⚙️`,detail:`Run the task asynchronously. The client does not need to poll or keep a connection open — it can shut down and come back later.`},{label:`Deliver Notification`,icon:`📬`,detail:`On completion (or failure), POST a TaskStatusUpdateEvent JSON body to the registered webhook URL. Include the full task object with artifacts.`},{label:`Sign Payload`,icon:`🔏`,detail:`HMAC-sign the notification body with a shared secret so the receiving agent can verify authenticity. Reject unsigned or tampered webhooks.`},{label:`Retry on Failure`,icon:`🔁`,detail:`If the webhook POST fails (non-2xx or timeout), retry with exponential backoff — 1s, 2s, 4s, up to 5 attempts. Log permanently failed deliveries for debugging.`}],code:`import express from "express";
...
app.listen(3000);
clientApp.listen(3001);`},{id:`a2a-auth`,category:`Production`,title:`Authentication & Security`,difficulty:`Advanced`,time:`~45 min`,description:`Secure agent-to-agent communication with Bearer tokens and OAuth 2.0 client credentials. Agents authenticate to each other without human involvement.`,tags:[`OAuth`,`bearer`,`security`,`M2M`],steps:[{label:`Read Auth from Card`,icon:`🔍`,detail:`Inspect the target agent's card authentication.schemes array before making any request. Only proceed if you support one of the declared schemes.`},{label:`Client Credentials`,icon:`🤝`,detail:`For machine-to-machine (M2M) auth, use OAuth 2.0 client credentials flow: POST to the token endpoint with client_id and client_secret to get a Bearer token.`},{label:`Cache Tokens`,icon:`💾`,detail:`Cache the access token and reuse it until expires_in seconds have elapsed. Fetching a new token per request is wasteful and may hit rate limits.`},{label:`Attach Bearer`,icon:`🔐`,detail:`Include Authorization: Bearer <token> on every A2A request. The receiving agent validates the token before processing — reject all unauthenticated requests with 401.`},{label:`Validate on Server`,icon:`✅`,detail:`On the server, verify the JWT signature, issuer (iss), audience (aud), and expiry (exp). Use a JWKS endpoint for key rotation without redeployment.`},{label:`Scope Enforcement`,icon:`🛡️`,detail:`Encode allowed skills in token scopes (e.g. a2a:summarize-dataset). Reject requests for skills not covered by the presented token's scopes.`}],code:`import express from "express";
import jwt from "jsonwebtoken";
...
app.listen(3000);`}],Hr=[`All`,`Foundations`,`Streaming`,`Multi-Agent`,`Production`],Ur={Beginner:`#0F6E56`,Intermediate:`#185FA5`,Advanced:`#993C1D`},Wr={Beginner:`#E1F5EE`,Intermediate:`#E6F1FB`,Advanced:`#FAECE7`};function Gr({steps:e}){let[t,n]=(0,x.useState)(null);return(0,N.jsxs)(`div`,{style:{marginTop:16},children:[(0,N.jsx)(`div`,{style:{display:`flex`,flexWrap:`wrap`,gap:8,alignItems:`center`},children:e.map((r,i)=>(0,N.jsxs)(`div`,{style:{display:`flex`,alignItems:`center`,gap:6},children:[(0,N.jsxs)(`button`,{onClick:()=>n(t===i?null:i),style:{display:`flex`,alignItems:`center`,gap:6,padding:`6px 12px`,borderRadius:20,border:t===i?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:t===i?`#E6F1FB`:`var(--color-background-primary)`,color:t===i?`#185FA5`:`var(--color-text-primary)`,cursor:`pointer`,fontSize:13,fontWeight:t===i?500:400,transition:`all 0.15s`},children:[(0,N.jsx)(`span`,{children:r.icon}),(0,N.jsx)(`span`,{children:r.label})]}),i<e.length-1&&(0,N.jsx)(`span`,{style:{color:`var(--color-text-tertiary)`,fontSize:12},children:`→`})]},i))}),t!==null&&(0,N.jsxs)(`div`,{style:{marginTop:10,padding:`10px 14px`,borderRadius:8,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.6},children:[(0,N.jsxs)(`span`,{style:{fontWeight:500,color:`var(--color-text-primary)`},children:[e[t].label,`: `]}),e[t].detail]})]})}function Kr({code:e}){let[t,n]=(0,x.useState)(!1);return(0,N.jsxs)(`div`,{style:{position:`relative`,marginTop:16},children:[(0,N.jsx)(`button`,{onClick:()=>{navigator.clipboard.writeText(e),n(!0),setTimeout(()=>n(!1),1800)},style:{position:`absolute`,top:8,right:8,padding:`4px 10px`,borderRadius:6,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,cursor:`pointer`,fontSize:12,color:`var(--color-text-secondary)`,zIndex:1},children:t?`✓ Copied`:`Copy`}),(0,N.jsx)(`pre`,{style:{margin:0,padding:`14px 16px`,borderRadius:10,overflowX:`auto`,background:`var(--color-background-secondary)`,border:`0.5px solid var(--color-border-tertiary)`,fontSize:12,lineHeight:1.65,fontFamily:`var(--font-mono)`,color:`var(--color-text-primary)`,whiteSpace:`pre`},children:(0,N.jsx)(`code`,{children:e})})]})}function qr({recipe:e,onSelect:t,selected:n}){return(0,N.jsxs)(`div`,{onClick:()=>t(e),style:{padding:`16px 18px`,borderRadius:12,cursor:`pointer`,border:n?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:n?`#061320`:`var(--color-background-primary)`,transition:`all 0.15s`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:6},children:[(0,N.jsx)(`span`,{style:{fontSize:13,color:`var(--color-text-secondary)`,fontWeight:400},children:e.category}),(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,fontWeight:500,background:Wr[e.difficulty],color:Ur[e.difficulty]},children:e.difficulty})]}),(0,N.jsx)(`div`,{style:{fontWeight:500,fontSize:15,marginBottom:4,color:`var(--color-text-primary)`},children:e.title}),(0,N.jsx)(`div`,{style:{fontSize:13,color:`var(--color-text-secondary)`,lineHeight:1.5},children:e.description}),(0,N.jsx)(`div`,{style:{marginTop:10,display:`flex`,gap:6,flexWrap:`wrap`},children:e.tags.map(e=>(0,N.jsx)(`span`,{style:{fontSize:11,padding:`2px 8px`,borderRadius:20,background:`var(--color-background-tertiary)`,color:`var(--color-text-secondary)`,border:`0.5px solid var(--color-border-tertiary)`},children:e},e))})]})}function Jr({recipe:e}){let[t,n]=(0,x.useState)(`steps`);return(0,N.jsxs)(`div`,{style:{padding:`24px`,borderRadius:14,background:`var(--color-background-primary)`,border:`0.5px solid var(--color-border-tertiary)`},children:[(0,N.jsxs)(`div`,{style:{display:`flex`,justifyContent:`space-between`,alignItems:`flex-start`,marginBottom:4},children:[(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:e.category}),(0,N.jsx)(`h2`,{style:{margin:`4px 0 6px`,fontSize:22,fontWeight:500},children:e.title})]}),(0,N.jsxs)(`div`,{style:{display:`flex`,gap:8,alignItems:`center`,paddingTop:4},children:[(0,N.jsx)(`span`,{style:{fontSize:12,padding:`3px 10px`,borderRadius:20,fontWeight:500,background:Wr[e.difficulty],color:Ur[e.difficulty]},children:e.difficulty}),(0,N.jsxs)(`span`,{style:{fontSize:12,color:`var(--color-text-tertiary)`},children:[`⏱ `,e.time]})]})]}),(0,N.jsx)(`p`,{style:{margin:`0 0 20px`,color:`var(--color-text-secondary)`,fontSize:14,lineHeight:1.6},children:e.description}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:4,marginBottom:18,borderBottom:`0.5px solid var(--color-border-tertiary)`,paddingBottom:0},children:[`steps`,`code`].map(e=>(0,N.jsx)(`button`,{onClick:()=>n(e),style:{padding:`8px 16px`,border:`none`,background:`none`,cursor:`pointer`,fontSize:14,fontWeight:t===e?500:400,color:t===e?`var(--color-text-primary)`:`var(--color-text-secondary)`,borderBottom:t===e?`2px solid #185FA5`:`2px solid transparent`,marginBottom:-1,transition:`all 0.12s`},children:e===`steps`?`Pipeline Steps`:`Code`},e))}),t===`steps`&&(0,N.jsx)(Gr,{steps:e.steps}),t===`code`&&(0,N.jsx)(Kr,{code:e.code})]})}function Yr({recipes:e,selected:t,onSelect:n,category:r,setCategory:i,search:a,setSearch:o}){let s=e.filter(e=>{let t=r===`All`||e.category===r,n=e.title.toLowerCase().includes(a.toLowerCase())||e.tags.some(e=>e.toLowerCase().includes(a.toLowerCase()));return t&&n});return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100%`,gap:0},children:[(0,N.jsx)(`div`,{style:{padding:`0 0 16px`},children:(0,N.jsx)(`input`,{type:`text`,placeholder:`Search recipes…`,value:a,onChange:e=>o(e.target.value),style:{width:`100%`,boxSizing:`border-box`,padding:`8px 12px`,borderRadius:8,border:`0.5px solid var(--color-border-secondary)`,background:`var(--color-background-secondary)`,color:`var(--color-text-primary)`,fontSize:13}})}),(0,N.jsx)(`div`,{style:{display:`flex`,gap:6,flexWrap:`wrap`,marginBottom:16},children:Hr.map(e=>(0,N.jsx)(`button`,{onClick:()=>i(e),style:{padding:`4px 12px`,borderRadius:20,fontSize:12,cursor:`pointer`,border:r===e?`1.5px solid #185FA5`:`0.5px solid var(--color-border-tertiary)`,background:r===e?`#E6F1FB`:`var(--color-background-primary)`,color:r===e?`#185FA5`:`var(--color-text-secondary)`,fontWeight:r===e?500:400},children:e},e))}),(0,N.jsx)(`div`,{style:{display:`flex`,flexDirection:`column`,gap:10,overflowY:`auto`,flex:1},children:s.length===0?(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,fontSize:13,padding:`12px 0`},children:`No recipes found.`}):s.map(e=>(0,N.jsx)(qr,{recipe:e,onSelect:n,selected:t?.id===e.id},e.id))})]})}function Xr(){return(0,N.jsxs)(`div`,{style:{padding:`20px 32px 16px`,borderBottom:`0.5px solid var(--color-border-tertiary)`,display:`flex`,alignItems:`center`,gap:16},children:[(0,N.jsx)(`div`,{style:{width:40,height:40,borderRadius:10,background:`#E6F1FB`,display:`flex`,alignItems:`center`,justifyContent:`center`,fontSize:20},children:`🤝`}),(0,N.jsxs)(`div`,{children:[(0,N.jsx)(`h1`,{style:{margin:0,fontSize:20,fontWeight:500,letterSpacing:`-0.3px`},children:`A2A Cookbook`}),(0,N.jsx)(`p`,{style:{margin:0,fontSize:13,color:`var(--color-text-secondary)`},children:`End-to-end Agent-to-Agent protocol recipes`})]}),(0,N.jsx)(`div`,{style:{marginLeft:`auto`,display:`flex`,gap:20},children:[{label:`Recipes`,value:Vr.length},{label:`Patterns`,value:Hr.length-1}].map(({label:e,value:t})=>(0,N.jsxs)(`div`,{style:{textAlign:`center`},children:[(0,N.jsx)(`div`,{style:{fontSize:18,fontWeight:500},children:t}),(0,N.jsx)(`div`,{style:{fontSize:11,color:`var(--color-text-tertiary)`},children:e})]},e))})]})}function Zr(){let[e,t]=(0,x.useState)(Vr[0]),[n,r]=(0,x.useState)(`All`),[i,a]=(0,x.useState)(``);return(0,N.jsxs)(`div`,{style:{display:`flex`,flexDirection:`column`,height:`100vh`,fontFamily:`var(--font-sans, system-ui, sans-serif)`,background:`var(--color-background-tertiary, #152060)`,color:`var(--color-text-primary)`},children:[(0,N.jsx)(Xr,{}),(0,N.jsxs)(`div`,{style:{display:`flex`,flex:1,overflow:`hidden`},children:[(0,N.jsx)(`div`,{style:{width:320,minWidth:260,padding:`20px 20px`,borderRight:`0.5px solid var(--color-border-tertiary)`,background:`var(--color-background-primary)`,overflowY:`auto`},children:(0,N.jsx)(Yr,{recipes:Vr,selected:e,onSelect:t,category:n,setCategory:r,search:i,setSearch:a})}),(0,N.jsx)(`div`,{style:{flex:1,overflowY:`auto`,padding:`24px 28px`},children:e?(0,N.jsx)(Jr,{recipe:e}):(0,N.jsx)(`div`,{style:{color:`var(--color-text-tertiary)`,padding:40,textAlign:`center`},children:`Select a recipe to get started`})})]})]})}var Qr=[{id:`credit-risk`,title:`Credit Risk Assessment System`,category:`Machine Learning | Classification | MLOps`,description:`An end-to-end cloud-native ML system that assesses credit risk of loan applicants using machine learning models and real-time scoring APIs.`,problemStatement:`Evaluate loan applicant risk to reduce financial defaults and automate credit approval decisions using machine learning.`,architecture:`/IntelliCatalyst-Labs/assets/Credit-risk-assessment-system-H9JeCkFP.png`,workflow:[`Data ingestion from banking and credit sources`,`Data cleaning and preprocessing`,`Exploratory Data Analysis (EDA)`,`Feature engineering (DTI ratio, credit utilization, etc.)`,`Feature selection and transformation`,`Model training (Logistic Regression, Random Forest, XGBoost)`,`Model evaluation using ROC-AUC`,`Risk scoring (Low / Medium / High)`,`Model registration using MLflow`,`Deployment using FastAPI + Docker + Kubernetes`],deployment:`Cloud-native API-based scoring system deployed using FastAPI, Docker, and Kubernetes with CI/CD pipeline integration.`,github:`https://github.com/Pooja-AI/credit-risk-assessment-system`,demo:``,documentation:`/IntelliCatalyst-Labs/assets/Credit-risk-assessment-system-BHdroidS.pdf`,tech:[`Python`,`Scikit-Learn`,`XGBoost`,`FastAPI`,`Docker`,`Kubernetes`,`MLflow`,`PostgreSQL`,`Redis`,`React`]},{id:`email-classification`,title:`Enterprise Email Classification System`,category:`Natural Language Processing`,description:`Automated email classification system using traditional NLP techniques and Machine Learning algorithms to categorize incoming emails into predefined classes.`,problemStatement:`Organizations receive thousands of emails daily. Manually categorizing emails is time-consuming and error-prone. This system automatically classifies emails into predefined categories using NLP and Machine Learning.`,architecture:`/IntelliCatalyst-Labs/assets/email_classification_system_architecture-CkIQcp5j.png`,workflow:[`Email data collection`,`Text preprocessing and cleaning`,`Tokenization and stopword removal`,`Feature extraction using CountVectorizer`,`Feature extraction using TF-IDF`,`Model training (Naive Bayes, SVM, Random Forest, Decision Tree, XGBoost, KNN, Logistic Regression)`,`Model evaluation and comparison`,`Model serialization using Pickle`,`REST API development using Flask`,`Web UI deployment using HTML, CSS, and JavaScript`],deployment:`Deployed as a Flask-based web application with REST APIs, integrated with a responsive frontend built using HTML, CSS, and JavaScript for real-time email classification.`,github:`https://github.com/Pooja-AI/Email-Classification`,demo:``,documentation:`/IntelliCatalyst-Labs/assets/Email_Classification_System-Dm6yGDMR.pdf`,tech:[`Python`,`Scikit-Learn`,`NLP`,`CountVectorizer`,`TF-IDF`,`Naive Bayes`,`SVM`,`Random Forest`,`Decision Tree`,`XGBoost`,`KNN`,`Logistic Regression`,`Flask`,`REST API`,`HTML`,`CSS`,`JavaScript`]}];function $r({project:e}){return(0,N.jsx)(M,{to:`/machine-learning/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/machine-learning/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function ei(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Machine Learning Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Qr.map(e=>(0,N.jsx)($r,{project:e},e.id))})]})}function ti(){let{id:e}=ot(),t=Qr.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var ni=[{id:`cnn-image-classification`,title:`Image Classification using CNN`,category:`Computer Vision`,description:`Deep learning system for classifying images using Convolutional Neural Networks (CNN).`,problemStatement:`Manual image classification is inefficient and error-prone. This system automates image recognition using CNN-based deep learning models.`,architecture:`/architecture/cnn-classification.png`,workflow:[`Image data collection`,`Preprocessing and augmentation`,`CNN model design`,`Model training`,`Evaluation and prediction`],deployment:`Deployed using TensorFlow model serving with Python API backend.`,github:``,demo:``,documentation:``,tech:[`Python`,`TensorFlow`,`Keras`,`CNN`]},{id:`yolo-object-detection`,title:`Real-Time Object Detection using YOLO`,category:`Computer Vision`,description:`Deep learning system for detecting multiple objects in real-time using YOLO architecture.`,problemStatement:`Traditional image classification cannot identify multiple objects in real-time scenes.`,architecture:`/architecture/yolo.png`,workflow:[`Video/image input capture`,`Frame preprocessing`,`YOLO model inference`,`Bounding box detection`,`Result visualization`],deployment:`Deployed using OpenCV + YOLO inference pipeline with real-time video stream.`,github:``,demo:``,documentation:``,tech:[`Python`,`YOLO`,`OpenCV`]},{id:`face-recognition`,title:`Face Recognition and Verification System`,category:`Computer Vision`,description:`Deep learning system for face detection, recognition, and verification.`,problemStatement:`Security systems require reliable and automated identity verification methods.`,architecture:`/architecture/face-recognition.png`,workflow:[`Face detection`,`Feature extraction`,`Embedding generation`,`Face matching`,`Identity verification`],deployment:`Deployed using FaceNet embeddings with API-based verification system.`,github:``,demo:``,documentation:``,tech:[`Python`,`FaceNet`,`OpenCV`]},{id:`medical-imaging`,title:`Medical Image Diagnosis using Deep Learning`,category:`Healthcare AI`,description:`AI system for detecting diseases from medical images using CNN models.`,problemStatement:`Manual medical image diagnosis is time-consuming and requires expert analysis.`,architecture:`/architecture/medical-dl.png`,workflow:[`Medical image ingestion`,`Preprocessing`,`CNN model training`,`Feature extraction`,`Disease prediction`],deployment:`Deployed in healthcare AI pipeline with secure model inference API.`,github:``,demo:``,documentation:``,tech:[`Python`,`CNN`,`TensorFlow`]},{id:`lane-detection`,title:`Autonomous Driving Lane Detection System`,category:`Computer Vision`,description:`Deep learning system for detecting road lanes in autonomous driving.`,problemStatement:`Autonomous vehicles require accurate lane detection for safe navigation.`,architecture:`/architecture/lane-detection.png`,workflow:[`Road image capture`,`Edge detection`,`Lane segmentation`,`Model prediction`,`Overlay visualization`],deployment:`Deployed using OpenCV + deep learning pipeline for real-time lane detection.`,github:``,demo:``,documentation:``,tech:[`Python`,`OpenCV`,`Deep Learning`]},{id:`mnist-digit`,title:`Handwritten Digit Recognition (MNIST)`,category:`Computer Vision`,description:`Deep learning model for recognizing handwritten digits.`,problemStatement:`Manual digit recognition systems are inefficient for large-scale input processing.`,architecture:`/architecture/mnist.png`,workflow:[`Dataset loading (MNIST)`,`Data preprocessing`,`CNN model training`,`Evaluation`,`Digit prediction`],deployment:`Deployed using TensorFlow/Keras inference API.`,github:``,demo:``,documentation:``,tech:[`Python`,`TensorFlow`,`MNIST`]},{id:`speech-emotion`,title:`Speech Emotion Recognition System`,category:`Audio AI`,description:`Deep learning system to detect emotions from speech signals.`,problemStatement:`Understanding human emotions from speech is important for conversational AI systems.`,architecture:`/architecture/speech-emotion.png`,workflow:[`Audio preprocessing`,`Feature extraction (MFCC)`,`LSTM model training`,`Emotion classification`,`Result visualization`],deployment:`Deployed using LSTM-based audio classification pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`LSTM`,`Librosa`]},{id:`text-to-speech`,title:`Text-to-Speech Conversion System`,category:`NLP + Audio`,description:`Deep learning system to convert text into natural human speech.`,problemStatement:`Accessibility systems require natural-sounding speech synthesis from text.`,architecture:`/architecture/tts.png`,workflow:[`Text preprocessing`,`Phoneme conversion`,`Neural speech synthesis`,`Audio generation`,`Playback output`],deployment:`Deployed using neural TTS engine with API interface.`,github:``,demo:``,documentation:``,tech:[`Python`,`TTS`,`Deep Learning`]},{id:`neural-translation`,title:`Neural Machine Translation System`,category:`NLP`,description:`Deep learning system for language translation using seq2seq models.`,problemStatement:`Traditional translation systems lack contextual understanding.`,architecture:`/architecture/translation.png`,workflow:[`Text preprocessing`,`Tokenization`,`Seq2Seq model training`,`Attention mechanism`,`Translation generation`],deployment:`Deployed using LSTM-based translation pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`Seq2Seq`,`LSTM`]},{id:`chatbot-seq2seq`,title:`Chatbot using Sequence-to-Sequence Models`,category:`NLP`,description:`Deep learning chatbot built using seq2seq architecture.`,problemStatement:`Rule-based chatbots lack flexibility and contextual understanding.`,architecture:`/architecture/chatbot.png`,workflow:[`Conversation dataset preparation`,`Tokenization`,`Seq2Seq training`,`Response generation`,`Dialogue flow`],deployment:`Deployed as conversational AI API service.`,github:``,demo:``,documentation:``,tech:[`Python`,`Seq2Seq`,`TensorFlow`]},{id:`dl-sentiment`,title:`Sentiment Analysis using Deep Learning`,category:`NLP`,description:`Deep learning system for sentiment classification of text.`,problemStatement:`Understanding customer sentiment is critical for business intelligence.`,architecture:`/architecture/sentiment.png`,workflow:[`Text preprocessing`,`Embedding generation`,`LSTM model training`,`Sentiment classification`,`Result visualization`],deployment:`Deployed using LSTM-based NLP pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`LSTM`,`Deep Learning`]}];function ri({project:e}){return(0,N.jsx)(M,{to:`/deep-learning/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/deep-learning/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function ii(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Deep Learning Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:ni.map(e=>(0,N.jsx)(ri,{project:e},e.id))})]})}function ai(){let{id:e}=ot(),t=ni.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var oi=[{id:`grid-world-qlearning`,title:`Grid World Path Finder using Q-Learning`,category:`Value-Based RL`,description:`Find optimal path in grid world using Q-Learning algorithm.`,problemStatement:`Design a reinforcement learning agent that learns the optimal path from a start state to a goal state while avoiding obstacles and maximizing cumulative rewards.`,architecture:`Environment → State Space → Q-Table → Action Selection (Epsilon-Greedy) → Reward System → Policy Update → Optimal Path Extraction`,workflow:[`Initialize grid world environment`,`Define reward structure for goal, obstacle, and steps`,`Initialize Q-table`,`Apply epsilon-greedy exploration`,`Update Q-values iteratively`,`Derive optimal policy path`],deployment:`Deployed as a Python simulation using OpenAI Gym-style custom environment`,github:``,demo:``,documentation:``,tech:[`Python`,`Q-Learning`,`Reinforcement Learning`]},{id:`cliff-walking`,title:`Cliff Walking Problem (Safe vs Risky Path Learning)`,category:`Value-Based RL`,description:`Learn safe vs risky paths using reinforcement learning.`,problemStatement:`Train an RL agent to navigate from start to goal while avoiding cliff states that produce high negative rewards.`,architecture:`Cliff Environment → State Space → Q-Table → Policy (Epsilon-Greedy) → Reward Penalization → Learning Loop`,workflow:[`Define CliffWalking environment`,`Initialize Q-table`,`Apply epsilon-greedy strategy`,`Train over multiple episodes`,`Penalize cliff falls heavily`,`Converge to safe path policy`],deployment:`Gym-based simulation environment`,github:``,demo:``,documentation:``,tech:[`Python`,`Q-Learning`,`Gym Environment`]},{id:`cartpole-dqn`,title:`CartPole Balancing using Deep Q-Network (DQN)`,category:`Deep RL`,description:`Balance pole using Deep Q-Network agent.`,problemStatement:`Train a deep reinforcement learning agent to balance a pole on a moving cart for maximum duration without failure.`,architecture:`Environment → Neural Network (Q-function) → Experience Replay → Target Network → Action Selection → Training Loop`,workflow:[`Initialize CartPole environment`,`Build deep neural network model`,`Store experiences in replay buffer`,`Sample mini-batches for training`,`Update Q-network and target network`,`Evaluate balancing performance`],deployment:`Trained using PyTorch + OpenAI Gym simulation environment`,github:``,demo:``,documentation:``,tech:[`Python`,`DQN`,`PyTorch`]},{id:`taxi-v3`,title:`Taxi V3 Environment RL Agent`,category:`Value-Based RL`,description:`Train agent to solve taxi pickup and drop problem.`,problemStatement:`Develop an RL agent that learns optimal pickup and drop-off strategies in a grid-world taxi environment.`,architecture:`Taxi Environment → State Encoding → Q-Table → Action Selection → Reward System → Policy Optimization`,workflow:[`Define Taxi-v3 environment`,`Encode states (location, passenger, destination)`,`Initialize Q-table`,`Train using Q-learning`,`Optimize pickup/drop strategy`,`Evaluate success rate`],deployment:`OpenAI Gym Taxi-v3 simulation`,github:``,demo:``,documentation:``,tech:[`Python`,`OpenAI Gym`,`Q-Learning`]},{id:`stock-trading-rl`,title:`Stock Trading Simulator using RL`,category:`Finance RL`,description:`Simulate stock trading decisions using reinforcement learning.`,problemStatement:`Build an RL agent that learns optimal buy/sell/hold decisions to maximize profit in stock markets.`,architecture:`Market Data → Feature Engineering → Trading Environment → RL Agent → Reward (Profit/Loss) → Policy Learning`,workflow:[`Load historical stock data`,`Define trading actions (buy/sell/hold)`,`Create custom trading environment`,`Define reward based on profit/loss`,`Train RL agent`,`Backtest strategy performance`],deployment:`Python-based backtesting simulation environment`,github:``,demo:``,documentation:``,tech:[`Python`,`RL`,`Finance`]}];function si({project:e}){return(0,N.jsx)(M,{to:`/reinforcement-learning/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/reinforcement-learning/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function ci(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Reinforcement Learning Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:oi.map(e=>(0,N.jsx)(si,{project:e},e.id))})]})}function li(){let{id:e}=ot(),t=oi.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var ui=[{id:`end-to-end-ml-pipeline`,title:`End-to-End Machine Learning Pipeline for Production Deployment`,category:`MLOps Pipeline`,description:`Complete machine learning pipeline from data ingestion to production deployment.`,problemStatement:`Machine learning models often fail in production due to lack of standardized pipelines and automation.`,architecture:`/architecture/ml-pipeline.png`,workflow:[`Data ingestion`,`Data preprocessing`,`Model training`,`Model evaluation`,`Model deployment`,`Monitoring and retraining`],deployment:`Deployed using Dockerized ML pipeline with CI/CD automation.`,github:``,demo:``,documentation:``,tech:[`Python`,`MLOps`,`CI/CD`,`Docker`]},{id:`automated-retraining-pipeline`,title:`Automated Model Training and Retraining Pipeline`,category:`Automation`,description:`Pipeline that automatically retrains models based on new data or drift detection.`,problemStatement:`Static ML models degrade over time without retraining on new data.`,architecture:`/architecture/retraining.png`,workflow:[`Data monitoring`,`Drift detection`,`Trigger retraining`,`Model training`,`Validation`,`Deployment`],deployment:`Deployed using Airflow-based orchestration pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`Airflow`,`MLflow`]},{id:`ml-ci-cd-pipeline`,title:`CI/CD Pipeline for Machine Learning Models`,category:`CI/CD`,description:`Automated CI/CD pipeline for ML model testing, validation, and deployment.`,problemStatement:`Manual deployment of ML models leads to inconsistencies and errors in production.`,architecture:`/architecture/ml-cicd.png`,workflow:[`Code commit`,`Automated testing`,`Model validation`,`Docker build`,`Deployment`,`Monitoring`],deployment:`Deployed using GitHub Actions + Docker CI/CD pipeline.`,github:``,demo:``,documentation:``,tech:[`GitHub Actions`,`Docker`,`MLflow`]},{id:`model-versioning-system`,title:`Model Versioning and Experiment Tracking System`,category:`Model Management`,description:`System for tracking ML experiments and managing model versions.`,problemStatement:`Lack of experiment tracking leads to poor reproducibility in ML projects.`,architecture:`/architecture/model-versioning.png`,workflow:[`Experiment tracking`,`Metric logging`,`Model versioning`,`Model comparison`,`Registry storage`],deployment:`Deployed using MLflow and DVC for experiment tracking.`,github:``,demo:``,documentation:``,tech:[`MLflow`,`DVC`,`Python`]},{id:`data-ingestion-pipeline`,title:`Scalable Data Ingestion and Preprocessing Pipeline`,category:`Data Engineering`,description:`Pipeline for scalable ingestion and preprocessing of large datasets.`,problemStatement:`Raw data from multiple sources is inconsistent and difficult to process.`,architecture:`/architecture/data-ingestion.png`,workflow:[`Data extraction`,`Data validation`,`Cleaning and transformation`,`Feature engineering`,`Storage in data lake`],deployment:`Deployed using Spark-based distributed data processing pipeline.`,github:``,demo:``,documentation:``,tech:[`Spark`,`Python`,`ETL`]},{id:`real-time-inference-api`,title:`Real-Time Model Inference API`,category:`Deployment`,description:`API system for serving ML models in real-time.`,problemStatement:`Batch prediction systems are not suitable for real-time applications.`,architecture:`/architecture/inference-api.png`,workflow:[`Input request`,`Data preprocessing`,`Model inference`,`Response generation`,`API output`],deployment:`Deployed using FastAPI + Docker for real-time inference.`,github:``,demo:``,documentation:``,tech:[`FastAPI`,`Docker`,`Python`]},{id:`model-monitoring-system`,title:`Model Monitoring System (Drift + Performance Tracking)`,category:`Monitoring`,description:`System to monitor ML model drift, latency, and performance.`,problemStatement:`Models in production degrade without proper monitoring systems.`,architecture:`/architecture/model-monitoring.png`,workflow:[`Data logging`,`Performance tracking`,`Drift detection`,`Alert generation`,`Dashboard visualization`],deployment:`Deployed using Prometheus + Grafana monitoring stack.`,github:``,demo:``,documentation:``,tech:[`Prometheus`,`Grafana`,`MLflow`]},{id:`automated-feature-engineering`,title:`Automated Feature Engineering Pipeline`,category:`Feature Engineering`,description:`System that automatically generates and selects features for ML models.`,problemStatement:`Manual feature engineering is time-consuming and error-prone.`,architecture:`/architecture/feature-engineering.png`,workflow:[`Raw data input`,`Feature generation`,`Feature selection`,`Feature validation`,`Model input preparation`],deployment:`Deployed using Featuretools-based automated pipeline.`,github:``,demo:``,documentation:``,tech:[`Featuretools`,`Python`]},{id:`multi-env-deployment`,title:`Multi-Environment ML Deployment Pipeline`,category:`Deployment`,description:`Pipeline to deploy ML models across dev, staging, and production environments.`,problemStatement:`Single-environment deployment leads to instability in production ML systems.`,architecture:`/architecture/multi-env.png`,workflow:[`Model training`,`Dev deployment`,`Staging validation`,`Production rollout`,`Monitoring`],deployment:`Deployed using Docker + Kubernetes multi-environment setup.`,github:``,demo:``,documentation:``,tech:[`Docker`,`Kubernetes`,`CI/CD`]},{id:`model-registry-workflow`,title:`Model Registry and Approval Workflow System`,category:`Governance`,description:`System for model approval, versioning, and deployment governance.`,problemStatement:`ML models require approval workflows before production deployment.`,architecture:`/architecture/model-registry.png`,workflow:[`Model training`,`Validation`,`Approval workflow`,`Version tagging`,`Production release`],deployment:`Deployed using MLflow model registry system.`,github:``,demo:``,documentation:``,tech:[`MLflow`,`CI/CD`]},{id:`batch-prediction-pipeline`,title:`Batch Prediction Pipeline for Large-Scale Data`,category:`Batch Processing`,description:`System for running ML predictions on large datasets in batch mode.`,problemStatement:`Real-time inference is not efficient for large-scale offline data processing.`,architecture:`/architecture/batch-pipeline.png`,workflow:[`Data ingestion`,`Batch preprocessing`,`Model inference`,`Result storage`,`Reporting`],deployment:`Deployed using Spark-based batch processing system.`,github:``,demo:``,documentation:``,tech:[`Spark`,`Python`]},{id:`streaming-ml-pipeline`,title:`Real-Time Streaming ML Pipeline`,category:`Streaming`,description:`Pipeline for real-time ML predictions using streaming data.`,problemStatement:`Batch pipelines cannot handle real-time event-driven data.`,architecture:`/architecture/streaming.png`,workflow:[`Data streaming ingestion`,`Kafka processing`,`Real-time feature extraction`,`Model inference`,`Output streaming`],deployment:`Deployed using Kafka + Spark Streaming pipeline.`,github:``,demo:``,documentation:``,tech:[`Kafka`,`Spark Streaming`,`Python`]},{id:`data-validation-pipeline`,title:`Data Validation and Quality Check Pipeline`,category:`Data Quality`,description:`System to validate data quality before ML model training.`,problemStatement:`Poor data quality leads to unreliable ML model performance.`,architecture:`/architecture/data-validation.png`,workflow:[`Data ingestion`,`Schema validation`,`Missing value detection`,`Anomaly detection`,`Quality reporting`],deployment:`Deployed using Great Expectations validation framework.`,github:``,demo:``,documentation:``,tech:[`Great Expectations`,`Python`]}];function di({project:e}){return(0,N.jsx)(M,{to:`/mlops/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/mlops/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function fi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`MLOps Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:ui.map(e=>(0,N.jsx)(di,{project:e},e.id))})]})}function pi(){let{id:e}=ot(),t=ui.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var mi=[{id:`enterprise-knowledge-qa`,title:`Enterprise Knowledge Base Q&A System`,category:`RAG Systems`,description:`Enterprise-grade Q&A system over internal knowledge base using RAG architecture.`,problemStatement:`Organizations struggle to extract relevant insights from large internal knowledge bases. This system enables accurate, context-aware answers using Retrieval-Augmented Generation.`,architecture:`/architecture/rag-chatbot.png`,workflow:[`Document ingestion`,`Text chunking & preprocessing`,`Embedding generation`,`Vector database storage`,`Query retrieval`,`LLM response generation`],deployment:`Deployed using FastAPI + FAISS + React UI`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Vector DB`,`RAG`,`FastAPI`]},{id:`pdf-chat-assistant`,title:`PDF / Document Chat Assistant using Vector Search`,category:`Document Intelligence`,description:`Chat with PDF documents using embeddings and semantic vector search.`,problemStatement:`Users struggle to manually read large PDF documents. This system enables natural language interaction with documents using RAG-based retrieval.`,architecture:`/architecture/pdf-rag.png`,workflow:[`PDF ingestion & parsing`,`Text extraction`,`Chunking strategy`,`Embedding generation`,`FAISS vector storage`,`Query-based retrieval`,`LLM response generation`],deployment:`Hosted using FastAPI backend with FAISS vector database and React frontend`,github:``,demo:``,documentation:``,tech:[`Python`,`FAISS`,`Embeddings`,`RAG`]},{id:`multi-doc-research`,title:`Multi-Document Research Assistant`,category:`Research AI`,description:`AI system for analyzing and summarizing multiple documents using RAG pipeline.`,problemStatement:`Researchers need efficient ways to extract insights from multiple documents simultaneously.`,architecture:`/architecture/multi-doc-rag.png`,workflow:[`Multi-document ingestion`,`Data preprocessing`,`Embedding generation`,`Cross-document retrieval`,`Context aggregation`,`LLM summarization`],deployment:`Deployed using scalable RAG pipeline with vector database and API service`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Vector DB`,`RAG`]},{id:`customer-support-rag`,title:`Customer Support Knowledge Retrieval Bot`,category:`Enterprise Support AI`,description:`AI-powered system that retrieves answers from enterprise knowledge base for customer support automation.`,problemStatement:`Customer support teams spend significant time searching knowledge bases manually. This system automates retrieval of accurate answers.`,architecture:`/architecture/support-rag.png`,workflow:[`Support ticket ingestion`,`Knowledge base indexing`,`Embedding generation`,`Intent detection`,`Semantic retrieval`,`Response generation`],deployment:`Integrated into enterprise support systems via API and chatbot UI`,github:``,demo:``,documentation:``,tech:[`Python`,`RAG`,`Embeddings`,`LLMs`]},{id:`legal-rag`,title:`Legal Document Q&A System using RAG`,category:`Legal AI`,description:`Query legal documents and extract precise answers using AI-powered retrieval.`,problemStatement:`Legal professionals require fast and accurate retrieval from large legal document repositories.`,architecture:`/architecture/legal-rag.png`,workflow:[`Legal document ingestion`,`Text normalization`,`Embedding generation`,`Case law retrieval`,`Context ranking`,`LLM response generation`],deployment:`Secure RAG system with encrypted vector database`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Vector DB`,`RAG`]},{id:`healthcare-rag`,title:`Healthcare Report Understanding Assistant`,category:`Healthcare AI`,description:`AI system for analyzing medical reports and generating insights using RAG.`,problemStatement:`Healthcare professionals need faster interpretation of complex medical reports.`,architecture:`/architecture/healthcare-rag.png`,workflow:[`Medical report ingestion`,`Data anonymization`,`Embedding generation`,`Context retrieval`,`Medical summarization`,`Insight generation`],deployment:`Deployed in secure healthcare environment with compliance standards`,github:``,demo:``,documentation:``,tech:[`Python`,`RAG`,`LLMs`]},{id:`tech-doc-chat`,title:`Technical Documentation Chat System`,category:`Developer AI`,description:`Chat system for interacting with technical documentation using semantic search.`,problemStatement:`Developers waste time searching through large technical documentation.`,architecture:`/architecture/dev-doc-rag.png`,workflow:[`Documentation ingestion`,`Markdown parsing`,`Embedding generation`,`Semantic search`,`Context retrieval`,`Answer generation`],deployment:`Deployed as developer portal with API access`,github:``,demo:``,documentation:``,tech:[`Python`,`Embeddings`,`RAG`]},{id:`hr-policy-assistant`,title:`HR Policy and Employee Handbook Assistant`,category:`Enterprise HR AI`,description:`AI assistant for answering HR policy and employee handbook queries.`,problemStatement:`Employees struggle to find HR policy answers from large documents.`,architecture:`/architecture/hr-rag.png`,workflow:[`HR document ingestion`,`Policy segmentation`,`Embedding generation`,`Query matching`,`Context retrieval`,`Answer generation`],deployment:`Integrated into internal HR portal`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Vector DB`]},{id:`financial-rag`,title:`Financial Report Analysis Assistant using RAG`,category:`Finance AI`,description:`AI system for analyzing financial reports and extracting insights.`,problemStatement:`Financial analysts need faster insights from complex financial documents.`,architecture:`/architecture/finance-rag.png`,workflow:[`Financial report ingestion`,`Data extraction`,`Embedding generation`,`Trend analysis retrieval`,`Context aggregation`,`Insight generation`],deployment:`Deployed in secure financial analytics environment`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Finance NLP`,`RAG`]},{id:`research-paper-qa`,title:`Academic Research Paper Q&A System`,category:`Research AI`,description:`AI system for querying academic papers and extracting insights.`,problemStatement:`Researchers struggle to extract insights from large academic papers.`,architecture:`/architecture/research-rag.png`,workflow:[`Paper ingestion`,`Citation parsing`,`Embedding generation`,`Semantic retrieval`,`Cross-paper comparison`,`Answer generation`],deployment:`Deployed as research assistant tool`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Embeddings`,`RAG`]}];function hi({project:e}){return(0,N.jsx)(M,{to:`/retrieval-augmented-generation/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/retrieval-augmented-generation/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function gi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Retrieval-Augmented Generation Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:mi.map(e=>(0,N.jsx)(hi,{project:e},e.id))})]})}function _i(){let{id:e}=ot(),t=mi.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var vi=[{id:`end-to-end-llmops-pipeline`,title:`End-to-End LLMOps Pipeline for Enterprise Applications`,category:`LLMOps Pipeline`,description:`Complete lifecycle pipeline for building, deploying, and managing LLM applications at scale.`,problemStatement:`Enterprises struggle to manage LLM lifecycle from development to production reliably and at scale.`,architecture:`/architecture/llmops-pipeline.png`,workflow:[`Data ingestion`,`Prompt/Model development`,`Experiment tracking`,`CI/CD pipeline setup`,`Deployment`,`Monitoring & feedback loop`],deployment:`Deployed using Docker, Kubernetes, and CI/CD pipelines for scalable LLM application management.`,github:``,demo:``,documentation:``,tech:[`LLMs`,`MLOps`,`CI/CD`,`Docker`,`Kubernetes`]},{id:`prompt-lifecycle-management`,title:`Prompt Lifecycle Management System`,category:`Prompt Engineering Ops`,description:`System for versioning, testing, and deploying prompts in production environments.`,problemStatement:`Prompts in production systems lack version control and systematic evaluation.`,architecture:`/architecture/prompt-lifecycle.png`,workflow:[`Prompt creation`,`Version control`,`A/B testing`,`Performance evaluation`,`Production deployment`],deployment:`Deployed as a prompt management platform integrated with LLM APIs.`,github:``,demo:``,documentation:``,tech:[`Prompt Engineering`,`LLMs`,`Version Control`]},{id:`llm-fine-tuning-pipeline`,title:`LLM Fine-Tuning Pipeline with Experiment Tracking`,category:`Model Training Ops`,description:`Pipeline for fine-tuning LLMs with tracking and evaluation.`,problemStatement:`Fine-tuning LLMs without experiment tracking leads to inconsistent results.`,architecture:`/architecture/llm-finetune.png`,workflow:[`Dataset preparation`,`Tokenization`,`Model fine-tuning`,`Experiment tracking (MLflow)`,`Evaluation`,`Model registry`],deployment:`Deployed using HuggingFace + MLflow tracking pipeline.`,github:``,demo:``,documentation:``,tech:[`HuggingFace`,`MLflow`,`LLMs`,`Fine-Tuning`]},{id:`rag-monitoring-pipeline`,title:`RAG Pipeline with Continuous Evaluation and Monitoring`,category:`RAG Ops`,description:`System to continuously monitor and evaluate RAG pipeline performance.`,problemStatement:`RAG systems degrade over time without monitoring retrieval and generation quality.`,architecture:`/architecture/rag-monitoring.png`,workflow:[`Query logging`,`Embedding retrieval tracking`,`LLM response evaluation`,`Drift detection`,`Performance monitoring dashboard`],deployment:`Deployed using monitoring tools like Prometheus and LLM evaluation pipelines.`,github:``,demo:``,documentation:``,tech:[`RAG`,`Vector DB`,`LLMs`,`Monitoring`]},{id:`llm-evaluation-framework`,title:`LLM Evaluation Framework for Response Quality`,category:`LLM Evaluation`,description:`Framework for evaluating LLM outputs and detecting hallucinations.`,problemStatement:`LLM outputs are not always reliable and require systematic evaluation.`,architecture:`/architecture/llm-eval.png`,workflow:[`Response collection`,`Ground truth comparison`,`Metric calculation`,`Hallucination detection`,`Scoring system`],deployment:`Deployed as evaluation service integrated with LLM pipelines.`,github:``,demo:``,documentation:``,tech:[`LLMs`,`Evaluation Metrics`,`NLP`]},{id:`llm-ci-cd-deployment`,title:`LLM Deployment Pipeline using CI/CD`,category:`Deployment Ops`,description:`Automated deployment system for LLM applications.`,problemStatement:`Manual deployment of LLM applications is error-prone and not scalable.`,architecture:`/architecture/llm-cicd.png`,workflow:[`Code commit`,`Automated testing`,`Docker build`,`Kubernetes deployment`,`Monitoring`],deployment:`Deployed using CI/CD pipelines with Docker and Kubernetes.`,github:``,demo:``,documentation:``,tech:[`Docker`,`Kubernetes`,`CI/CD`,`LLMs`]},{id:`ai-model-monitoring-system`,title:`AI Model Monitoring System`,category:`Monitoring Ops`,description:`System for tracking AI model performance, drift, and latency.`,problemStatement:`AI models in production degrade over time without monitoring systems.`,architecture:`/architecture/model-monitoring.png`,workflow:[`Log collection`,`Performance tracking`,`Drift detection`,`Alert system`,`Dashboard visualization`],deployment:`Deployed using Prometheus + MLflow monitoring stack.`,github:``,demo:``,documentation:``,tech:[`Monitoring`,`MLflow`,`Prometheus`,`LLMs`]},{id:`prompt-ab-testing`,title:`Prompt A/B Testing and Optimization Platform`,category:`Prompt Optimization`,description:`Platform for testing and optimizing prompt versions.`,problemStatement:`Prompt performance varies and needs systematic A/B testing.`,architecture:`/architecture/prompt-ab.png`,workflow:[`Prompt creation`,`A/B test setup`,`User interaction logging`,`Performance comparison`,`Optimization`],deployment:`Deployed as prompt experimentation platform.`,github:``,demo:``,documentation:``,tech:[`A/B Testing`,`LLMs`,`Analytics`]},{id:`llm-feedback-loop`,title:`LLM Feedback Loop System`,category:`Human-in-the-Loop AI`,description:`System to improve LLMs using human feedback.`,problemStatement:`LLMs require continuous improvement using real user feedback.`,architecture:`/architecture/llm-feedback.png`,workflow:[`User response collection`,`Feedback labeling`,`Model retraining`,`Evaluation`,`Improvement cycle`],deployment:`Deployed using RLHF-style feedback loop system.`,github:``,demo:``,documentation:``,tech:[`RLHF`,`LLMs`,`Feedback Systems`]},{id:`vector-db-lifecycle`,title:`Vector Database Lifecycle Management System`,category:`RAG Infrastructure`,description:`System for managing vector database indexing and updates.`,problemStatement:`Vector databases require lifecycle management for efficient retrieval.`,architecture:`/architecture/vector-db.png`,workflow:[`Data ingestion`,`Embedding generation`,`Index creation`,`Updates and optimization`,`Query handling`],deployment:`Deployed using FAISS / Pinecone / Weaviate infrastructure.`,github:``,demo:``,documentation:``,tech:[`FAISS`,`Pinecone`,`Weaviate`,`Embeddings`]},{id:`llm-routing-system`,title:`Multi-Model LLM Routing System`,category:`AI Optimization`,description:`System to route queries to best LLM based on cost and performance.`,problemStatement:`Different LLMs have different cost-performance tradeoffs.`,architecture:`/architecture/llm-routing.png`,workflow:[`Query analysis`,`Model selection`,`Routing logic`,`Response generation`,`Optimization feedback`],deployment:`Deployed as intelligent routing layer between LLM APIs.`,github:``,demo:``,documentation:``,tech:[`LLMs`,`Routing`,`Optimization`]},{id:`ai-governance-system`,title:`Enterprise AI Governance and Compliance System`,category:`AI Governance`,description:`System ensuring safety, compliance, and governance in AI workflows.`,problemStatement:`Enterprise AI systems require governance for compliance and safety.`,architecture:`/architecture/ai-governance.png`,workflow:[`Policy definition`,`Model auditing`,`Risk analysis`,`Compliance checks`,`Reporting`],deployment:`Deployed as enterprise AI governance framework.`,github:``,demo:``,documentation:``,tech:[`Governance`,`LLMs`,`Security`]}];function yi({project:e}){return(0,N.jsx)(M,{to:`/llmops/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/llmops/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function bi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`LLM Ops Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:vi.map(e=>(0,N.jsx)(yi,{project:e},e.id))})]})}function xi(){let{id:e}=ot(),t=vi.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Si=[{id:`personal-productivity-agent`,title:`AI Personal Productivity Agent`,category:`Autonomous Agents`,description:`Autonomous AI agent that plans, prioritizes, and executes personal productivity tasks using tool-calling and reasoning loops.`,problemStatement:`Individuals struggle to manage tasks efficiently across multiple tools and calendars. This agent automates planning and execution of daily workflows.`,architecture:`/architecture/productivity-agent.png`,workflow:[`User input task collection`,`Task prioritization`,`LLM-based planning`,`Tool selection and execution`,`Feedback loop optimization`],deployment:`Deployed as an autonomous agent system with API-based tool integration and LLM orchestration layer.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Tool Calling`,`Agents`]},{id:`autonomous-data-analysis`,title:`Autonomous Data Analysis Agent`,category:`Data Agents`,description:`AI agent that automatically fetches, analyzes, and generates structured reports from data sources.`,problemStatement:`Data analysts spend time manually processing and summarizing datasets. This agent automates end-to-end analysis.`,architecture:`/architecture/data-agent.png`,workflow:[`Data source connection`,`Data ingestion`,`Cleaning and preprocessing`,`Automated EDA`,`Insight generation`,`Report creation`],deployment:`Deployed using Python-based automation pipeline with LLM-powered analysis engine.`,github:``,demo:``,documentation:``,tech:[`Python`,`Pandas`,`LLMs`,`Automation`]},{id:`multi-agent-workflow`,title:`Multi-Agent Workflow Automation System`,category:`Multi-Agent Systems`,description:`Collaborative AI agents working together to execute complex enterprise workflows.`,problemStatement:`Single agents cannot efficiently handle complex multi-step workflows requiring specialization and coordination.`,architecture:`/architecture/multi-agent.png`,workflow:[`Task decomposition`,`Agent assignment`,`Parallel execution`,`Inter-agent communication`,`Result aggregation`],deployment:`Built using CrewAI / LangChain multi-agent orchestration framework.`,github:``,demo:``,documentation:``,tech:[`Python`,`CrewAI`,`LangChain`,`Agents`]},{id:`research-agent`,title:`AI Research Agent`,category:`Research Agents`,description:`Autonomous agent that searches, summarizes, and generates structured research reports.`,problemStatement:`Researchers spend significant time gathering and summarizing information manually from multiple sources.`,architecture:`/architecture/research-agent.png`,workflow:[`Query understanding`,`Web/document search`,`Information extraction`,`Summarization`,`Report generation`],deployment:`Deployed with web-search enabled LLM agent pipeline and structured report generator.`,github:``,demo:``,documentation:``,tech:[`Python`,`RAG`,`LLMs`,`Web Search`]},{id:`customer-support-agent`,title:`Customer Support Agent with Tool Calling`,category:`Enterprise Agents`,description:`AI agent that handles customer queries using tool calling, escalation logic, and knowledge retrieval.`,problemStatement:`Customer support teams face high response delays due to manual handling of repetitive queries.`,architecture:`/architecture/support-agent.png`,workflow:[`Query classification`,`Tool selection`,`Knowledge retrieval`,`Response generation`,`Escalation handling`],deployment:`Integrated into enterprise support systems with API-based tool execution.`,github:``,demo:``,documentation:``,tech:[`Python`,`Tool Calling`,`LLMs`]},{id:`sales-agent`,title:`AI Sales Agent`,category:`Business Agents`,description:`AI-powered sales automation agent for lead qualification and customer engagement.`,problemStatement:`Sales teams spend time manually qualifying leads and following up with prospects.`,architecture:`/architecture/sales-agent.png`,workflow:[`Lead ingestion`,`Lead scoring`,`Personalized messaging`,`Follow-up automation`,`CRM updates`],deployment:`Deployed with CRM integration and LLM-based communication engine.`,github:``,demo:``,documentation:``,tech:[`Python`,`CRM Automation`,`LLMs`]},{id:`devops-agent`,title:`AI DevOps Assistant Agent`,category:`DevOps Agents`,description:`Autonomous monitoring agent that detects system issues and suggests fixes.`,problemStatement:`DevOps teams need real-time system monitoring and intelligent failure detection.`,architecture:`/architecture/devops-agent.png`,workflow:[`System monitoring`,`Log analysis`,`Anomaly detection`,`Issue diagnosis`,`Fix recommendation`],deployment:`Integrated with monitoring systems and cloud infrastructure APIs.`,github:``,demo:``,documentation:``,tech:[`Python`,`Monitoring`,`LLMs`,`DevOps`]},{id:`travel-agent`,title:`Travel Planning Agent`,category:`Planning Agents`,description:`AI agent that generates travel itineraries and booking suggestions.`,problemStatement:`Travel planning is time-consuming and requires coordination across multiple platforms.`,architecture:`/architecture/travel-agent.png`,workflow:[`User preference collection`,`Destination analysis`,`Itinerary generation`,`Budget optimization`,`Booking suggestions`],deployment:`Deployed using API-based travel recommendation system with LLM planner.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`APIs`]},{id:`business-analyst-agent`,title:`AI Business Analyst Agent`,category:`Analytics Agents`,description:`AI agent that performs KPI analysis and generates business insights automatically.`,problemStatement:`Business analysts spend significant time manually generating reports and dashboards.`,architecture:`/architecture/business-agent.png`,workflow:[`Data ingestion`,`KPI computation`,`Trend analysis`,`Insight generation`,`Report creation`],deployment:`Deployed as BI automation layer integrated with dashboards.`,github:``,demo:``,documentation:``,tech:[`Python`,`Pandas`,`LLMs`,`BI`]},{id:`email-management-agent`,title:`Autonomous Email Management Agent`,category:`Productivity Agents`,description:`AI agent that sorts, prioritizes, and responds to emails intelligently.`,problemStatement:`Professionals struggle with email overload and prioritization.`,architecture:`/architecture/email-agent.png`,workflow:[`Email ingestion`,`Classification`,`Priority scoring`,`Auto-response generation`,`Scheduling follow-ups`],deployment:`Integrated with email APIs and LLM-based response engine.`,github:``,demo:``,documentation:``,tech:[`Python`,`Email APIs`,`LLMs`]}];function Ci({project:e}){return(0,N.jsx)(M,{to:`/agentic-ai/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/agentic-ai/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function wi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Agentic AI Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Si.map(e=>(0,N.jsx)(Ci,{project:e},e.id))})]})}function Ti(){let{id:e}=ot(),t=Si.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Ei=[{id:`text-classification`,title:`Text Classification System (News / Topic Classification)`,category:`Text Classification`,description:`AI system that classifies news articles or documents into predefined categories using NLP models.`,problemStatement:`Organizations struggle to automatically categorize large volumes of text data such as news, emails, and documents.`,architecture:`/architecture/nlp-text-classification.png`,workflow:[`Text data collection`,`Text preprocessing (cleaning, tokenization)`,`Feature extraction (TF-IDF / embeddings)`,`Model training (ML or transformer)`,`Prediction on unseen text`],deployment:`Deployed as REST API using FastAPI with model inference endpoint.`,github:``,demo:``,documentation:``,tech:[`Python`,`Scikit-Learn`,`NLP`]},{id:`spam-detection-nlp`,title:`Spam Email Detection System`,category:`Text Classification`,description:`Detects spam emails using machine learning and NLP-based text classification.`,problemStatement:`Email systems are flooded with spam messages that reduce productivity and security.`,architecture:`/architecture/nlp-spam-detection.png`,workflow:[`Email ingestion`,`Text preprocessing`,`Feature extraction`,`Spam classification model training`,`Real-time email prediction`],deployment:`Deployed as email filtering service integrated with backend API.`,github:``,demo:``,documentation:``,tech:[`Python`,`Naive Bayes`,`NLP`]},{id:`sentiment-analysis-reviews`,title:`Sentiment Analysis on Customer Reviews`,category:`Sentiment Analysis`,description:`Analyzes customer sentiment from product reviews using NLP and deep learning models.`,problemStatement:`Businesses need automated insights from customer reviews to understand sentiment at scale.`,architecture:`/architecture/nlp-sentiment-analysis.png`,workflow:[`Review data collection`,`Text cleaning and preprocessing`,`Sentiment feature extraction`,`Model training (LSTM / ML models)`,`Sentiment prediction`],deployment:`Deployed as analytics API for real-time sentiment scoring.`,github:``,demo:``,documentation:``,tech:[`Python`,`LSTM`,`TF-IDF`]},{id:`fake-news-detection`,title:`Fake News Detection System`,category:`Text Classification`,description:`Detects fake news articles using transformer-based NLP models.`,problemStatement:`Misinformation spreads quickly on digital platforms and needs automated detection systems.`,architecture:`/architecture/nlp-fake-news.png`,workflow:[`News data collection`,`Text preprocessing`,`Feature extraction`,`Transformer model training`,`Fake vs real classification`],deployment:`Deployed as API service integrated with web applications.`,github:``,demo:``,documentation:``,tech:[`Python`,`BERT`,`NLP`]},{id:`ner-system`,title:`Named Entity Recognition (NER) System`,category:`Information Extraction`,description:`Extracts entities like names, locations, and organizations from text.`,problemStatement:`Structured information extraction from unstructured text is essential for enterprise applications.`,architecture:`/architecture/nlp-ner.png`,workflow:[`Text ingestion`,`Tokenization`,`NER model inference`,`Entity extraction`,`Structured output generation`],deployment:`Deployed as NLP microservice with REST API.`,github:``,demo:``,documentation:``,tech:[`Python`,`SpaCy`,`Transformers`]},{id:`question-answering`,title:`Question Answering System using NLP`,category:`QA Systems`,description:`Answers user questions based on given document context using transformer models.`,problemStatement:`Users need fast and accurate answers from large documents without manual search.`,architecture:`/architecture/nlp-qa.png`,workflow:[`Context document ingestion`,`Question processing`,`Embedding generation`,`Context retrieval`,`Answer generation using transformer model`],deployment:`Deployed as intelligent QA API service.`,github:``,demo:``,documentation:``,tech:[`Python`,`BERT`,`Transformers`]},{id:`intent-chatbot`,title:`Chatbot using Intent Classification`,category:`Conversational AI`,description:`Conversational chatbot that understands user intent using NLP models.`,problemStatement:`Customer support requires automated conversational systems to handle repetitive queries.`,architecture:`/architecture/nlp-chatbot.png`,workflow:[`User input processing`,`Intent classification`,`Entity extraction`,`Response generation`,`Chat interaction loop`],deployment:`Deployed as chatbot service integrated into web applications.`,github:``,demo:``,documentation:``,tech:[`Python`,`LSTM`,`NLU`]},{id:`text-summarization`,title:`Document Summarization System (Extractive)`,category:`Text Summarization`,description:`Automatically summarizes long documents into concise key points.`,problemStatement:`Reading large documents is time-consuming and inefficient for users.`,architecture:`/architecture/nlp-summarization.png`,workflow:[`Document ingestion`,`Sentence segmentation`,`Importance scoring`,`Sentence ranking`,`Summary generation`],deployment:`Deployed as text summarization API service.`,github:``,demo:``,documentation:``,tech:[`Python`,`NLTK`,`Transformers`]},{id:`keyword-extraction`,title:`Keyword Extraction from Text Documents`,category:`Information Extraction`,description:`Extracts important keywords from large text documents using NLP techniques.`,problemStatement:`Manual keyword extraction from large documents is inefficient and error-prone.`,architecture:`/architecture/nlp-keywords.png`,workflow:[`Text preprocessing`,`TF-IDF computation`,`Keyword ranking`,`Top keyword selection`],deployment:`Deployed as lightweight NLP microservice.`,github:``,demo:``,documentation:``,tech:[`Python`,`TF-IDF`,`RAKE`]}];function Di({project:e}){return(0,N.jsx)(M,{to:`/natural-language-processing/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/natural-language-processing/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function Oi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Natural Language Processing Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Ei.map(e=>(0,N.jsx)(Di,{project:e},e.id))})]})}function ki(){let{id:e}=ot(),t=Ei.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Ai=[{id:`corrosion-detection-genai`,title:`Corrosion Detection GenAI - Vision + LLM Inspection Assistant`,category:`Computer Vision & GenAI`,description:`Multimodal AI system that detects corrosion in images of metal structures using a CNN/YOLO/ViT vision model, then uses an LLM to generate inspection reports and answer follow-up questions via a chat assistant.`,problemStatement:`Manual visual inspection of industrial assets (pipelines, tanks, beams, platforms) for corrosion is slow, inconsistent, and relies heavily on inspector expertise. This system automates corrosion detection from images and uses GenAI to produce structured severity assessments, risk levels, and maintenance recommendations.`,architecture:`/IntelliCatalyst-Labs/assets/corrosion-detection-DYsVYNgV.png`,workflow:[`Plant/asset selection and image upload`,`Image preprocessing and resizing`,`Vision model inference (corrosion detection + severity classification)`,`LLM reasoning over detections (risk level + inspection report)`,`Multimodal fusion of vision and LLM outputs`,`Interactive chat assistant for follow-up questions`],deployment:`Deployed as a FastAPI backend (vision model + LLM reasoning, config-driven via YAML) with a React (Vite) frontend; containerized with Docker and docker-compose for backend + frontend.`,github:`https://github.com/Pooja-AI/corrosion-detection-genai`,demo:``,documentation:`/IntelliCatalyst-Labs/assets/CorrosionDetectionDocs-DZRN5AH7.pdf`,tech:[`Python`,`FastAPI`,`React`,`PyTorch`,`YOLO/CNN/ViT`,`LLMs`,`Prompt Engineering`,`Docker`]},{id:`content-assistant`,title:`Smart Content Creation Assistant`,category:`Content Generation`,description:`AI-powered tool for generating blogs, articles, and long-form content.`,problemStatement:`Content creators need faster and high-quality content generation tools.`,architecture:`/architecture/content-gen.png`,workflow:[`Topic input`,`Prompt optimization`,`LLM content generation`,`SEO refinement`,`Final output formatting`],deployment:`Deployed as a web-based content generation tool using LLM APIs.`,github:``,demo:``,documentation:``,tech:[`Python`,`GPT`,`NLP`]},{id:`resume-optimizer`,title:`AI Resume Optimization Tool`,category:`Career AI`,description:`AI system to improve resumes with suggestions, formatting, and keyword optimization.`,problemStatement:`Job seekers struggle to optimize resumes for ATS and recruiter expectations.`,architecture:`/architecture/resume-ai.png`,workflow:[`Resume parsing`,`Skill extraction`,`LLM-based analysis`,`Improvement suggestions`,`Optimized resume generation`],deployment:`Deployed as SaaS-based resume optimization platform.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Prompt Engineering`]},{id:`meeting-summarizer`,title:`Meeting Notes Summarizer and Action Generator`,category:`Productivity AI`,description:`AI system to summarize meetings and extract actionable insights.`,problemStatement:`Meetings generate large amounts of unstructured notes that are difficult to process.`,architecture:`/architecture/meeting-summary.png`,workflow:[`Audio transcription`,`Text cleaning`,`Summarization using LLMs`,`Action item extraction`,`Output generation`],deployment:`Deployed using speech-to-text + LLM summarization pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Speech-to-Text`]},{id:`learning-generator`,title:`Personalized Learning Content Generator`,category:`Education AI`,description:`AI system that generates personalized learning materials for students.`,problemStatement:`Students need personalized learning content tailored to their skill level.`,architecture:`/architecture/learning-ai.png`,workflow:[`User profile analysis`,`Knowledge gap detection`,`Prompt generation`,`Content creation`,`Personalization layer`],deployment:`Deployed using RAG + LLM-based educational assistant system.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`RAG`]},{id:`marketing-copy`,title:`AI Marketing Copy Generator`,category:`Marketing AI`,description:`Generates marketing ads, social media posts, and promotional content.`,problemStatement:`Marketing teams require fast and creative copy generation for campaigns.`,architecture:`/architecture/marketing-gen.png`,workflow:[`Campaign input`,`Tone selection`,`Prompt engineering`,`Content generation`,`Optimization`],deployment:`Deployed using LLM-based marketing automation tool.`,github:``,demo:``,documentation:``,tech:[`Python`,`GPT`,`Prompt Engineering`]},{id:`story-generator`,title:`AI Story and Script Writing Assistant`,category:`Creative AI`,description:`Generates creative stories, scripts, and narrative content using AI.`,problemStatement:`Writers need creative assistance for story and script development.`,architecture:`/architecture/story-ai.png`,workflow:[`Idea input`,`Story structure generation`,`Character building`,`Narrative generation`,`Final script output`],deployment:`Deployed using LLM creative writing pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Creative Prompting`]},{id:`product-description`,title:`AI Product Description Generator`,category:`E-commerce AI`,description:`Generates SEO-friendly product descriptions for e-commerce platforms.`,problemStatement:`E-commerce sellers need scalable product description generation.`,architecture:`/architecture/product-desc.png`,workflow:[`Product input`,`Feature extraction`,`Prompt creation`,`LLM generation`,`SEO optimization`],deployment:`Deployed as SaaS tool for e-commerce content generation.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Prompt Engineering`]},{id:`code-explainer`,title:`Code Explanation and Documentation Generator`,category:`Developer AI`,description:`AI system that explains code and generates documentation automatically.`,problemStatement:`Developers spend significant time documenting and understanding code.`,architecture:`/architecture/code-ai.png`,workflow:[`Code input parsing`,`AST analysis`,`LLM explanation generation`,`Documentation formatting`,`Output generation`],deployment:`Deployed using LLM-based developer assistant tool.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Code Understanding`]},{id:`idea-generator`,title:`AI Startup Idea Generator`,category:`Innovation AI`,description:`Generates startup and project ideas using generative AI models.`,problemStatement:`Entrepreneurs struggle to generate innovative startup ideas consistently.`,architecture:`/architecture/idea-gen.png`,workflow:[`User interest input`,`Domain selection`,`LLM brainstorming`,`Idea ranking`,`Final idea output`],deployment:`Deployed using LLM-based idea generation engine.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Brainstorming AI`]}];function ji({project:e}){return(0,N.jsx)(M,{to:`/generative-ai/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/generative-ai/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function Mi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Generative AI Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Ai.map(e=>(0,N.jsx)(ji,{project:e},e.id))})]})}function Ni(){let{id:e}=ot(),t=Ai.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Pi=[{id:`end-to-end-ml-system-architecture`,title:`End-to-End Machine Learning System Architecture`,category:`ML Architecture`,description:`Complete ML lifecycle architecture from training to deployment and monitoring.`,problemStatement:`Design a scalable end-to-end ML system that handles data ingestion, training, deployment, and continuous monitoring in production environments.`,architecture:`Data Sources → Data Ingestion Layer → Feature Store → Training Pipeline → Model Registry → Deployment (API/Batch) → Monitoring & Feedback Loop`,workflow:[`Ingest raw data from multiple sources`,`Data preprocessing and feature engineering`,`Train ML models using pipelines`,`Register best model in model registry`,`Deploy model as API or batch service`,`Monitor drift and performance continuously`],deployment:`Deployed using Docker + Kubernetes with CI/CD pipelines and MLflow tracking`,github:``,demo:``,documentation:``,tech:[`ML Systems`,`MLOps`,`Cloud Architecture`]},{id:`real-time-ml-inference`,title:`Real-Time ML Inference System for Low-Latency Predictions`,category:`Inference Systems`,description:`Low-latency real-time prediction architecture for production ML systems.`,problemStatement:`Build a real-time inference system capable of serving ML predictions with low latency under high traffic loads.`,architecture:`Client Request → API Gateway → Load Balancer → Model Serving Layer → Cache (Redis) → ML Model → Response`,workflow:[`Receive real-time user request`,`Route request through API gateway`,`Fetch cached results if available`,`Send request to ML model server`,`Return prediction response`,`Log request for monitoring`],deployment:`Deployed using FastAPI + Redis + Kubernetes for scalable inference`,github:``,demo:``,documentation:``,tech:[`FastAPI`,`Kafka`,`Redis`,`ML Serving`]},{id:`batch-streaming-pipeline`,title:`Batch and Streaming Prediction Pipeline Architecture`,category:`Data Pipelines`,description:`Hybrid architecture combining batch and streaming ML predictions.`,problemStatement:`Design a hybrid data processing system that supports both real-time streaming and batch ML predictions.`,architecture:`Data Sources → Kafka (Streaming) + Data Lake (Batch) → Processing Layer (Spark) → Feature Store → ML Models → Output Layer`,workflow:[`Ingest streaming and batch data`,`Process data using Spark`,`Generate features for ML models`,`Run batch and streaming predictions`,`Store results in data warehouse`,`Serve results to applications`],deployment:`Deployed using Spark clusters + Kafka streaming + cloud storage`,github:``,demo:``,documentation:``,tech:[`Spark`,`Kafka`,`Airflow`]},{id:`feature-store-architecture`,title:`Feature Store Design for Scalable ML Systems`,category:`Feature Engineering Systems`,description:`Centralized feature store architecture for reusable ML features.`,problemStatement:`Create a centralized feature store that enables consistent, reusable, and real-time feature access across ML systems.`,architecture:`Data Sources → Feature Engineering Pipeline → Feature Store (Offline + Online) → Training + Inference Systems`,workflow:[`Collect raw data`,`Generate features using pipelines`,`Store features in offline store`,`Sync features to online store`,`Serve features for training and inference`,`Maintain versioning of features`],deployment:`Implemented using Feast + Redis + Snowflake`,github:``,demo:``,documentation:``,tech:[`Feast`,`Redis`,`Snowflake`]},{id:`model-monitoring-architecture`,title:`Model Monitoring and Drift Detection Architecture`,category:`Monitoring Systems`,description:`Detect drift and monitor ML models in production environments.`,problemStatement:`Design a monitoring system that tracks model performance, detects data drift, and triggers alerts for retraining.`,architecture:`Live Data → Prediction Service → Logging Layer → Monitoring Engine → Drift Detection → Alert System`,workflow:[`Capture real-time predictions`,`Log input and output data`,`Compare distributions over time`,`Detect drift using statistical methods`,`Trigger alerts if drift exceeds threshold`,`Initiate retraining pipeline`],deployment:`Deployed using Prometheus + Grafana + Evidently AI`,github:``,demo:``,documentation:``,tech:[`Prometheus`,`Grafana`,`Evidently AI`]},{id:`enterprise-rag-system`,title:`Enterprise RAG-Based Knowledge System Architecture`,category:`RAG Architecture`,description:`Enterprise-grade Retrieval-Augmented Generation system design.`,problemStatement:`Build a scalable enterprise RAG system that enables semantic search and accurate LLM-based answers over private knowledge bases.`,architecture:`Documents → Chunking → Embeddings → Vector DB → Retriever → LLM Generator → Response API`,workflow:[`Ingest enterprise documents`,`Split into chunks`,`Generate embeddings`,`Store in vector database`,`Retrieve relevant context`,`Generate response using LLM`],deployment:`Deployed using FastAPI + FAISS/Pinecone + LLM API`,github:``,demo:``,documentation:``,tech:[`Vector DB`,`LLMs`,`RAG`]},{id:`multi-agent-architecture`,title:`Multi-Agent AI System Architecture for Task Automation`,category:`Agent Systems`,description:`Architecture for multiple agents collaborating on tasks.`,problemStatement:`Design a multi-agent system where specialized agents collaborate to solve complex tasks autonomously.`,architecture:`User Input → Orchestrator Agent → Task Decomposition → Specialist Agents → Shared Memory → Final Output`,workflow:[`Receive user request`,`Orchestrator breaks task into subtasks`,`Assign subtasks to agents`,`Agents collaborate via shared memory`,`Aggregate outputs`,`Return final response`],deployment:`Deployed using LangGraph / CrewAI framework with API layer`,github:``,demo:``,documentation:``,tech:[`CrewAI`,`LangGraph`,`Agents`]}];function Fi({project:e}){return(0,N.jsx)(M,{to:`/system-design/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/system-design/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function Ii(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`System Design Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Pi.map(e=>(0,N.jsx)(Fi,{project:e},e.id))})]})}function Li(){let{id:e}=ot(),t=Pi.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Ri=[{id:`customer-behavior-analytics`,title:`Customer Behavior Analytics Dashboard`,category:`Customer Analytics`,description:`Advanced analytics dashboard to analyze customer behavior patterns and generate actionable business insights.`,problemStatement:`Businesses struggle to understand customer behavior patterns across multiple channels. This system provides data-driven insights for better decision making.`,architecture:`/architecture/customer-behavior.png`,workflow:[`Data collection from CRM systems`,`Data cleaning and preprocessing`,`Exploratory data analysis`,`Behavior segmentation`,`Insight visualization`],deployment:`Deployed using Power BI dashboards integrated with Python analytics backend.`,github:``,demo:``,documentation:``,tech:[`Python`,`Pandas`,`Matplotlib`,`Power BI`]},{id:`sales-performance-analytics`,title:`Sales Performance Analytics System`,category:`Sales Analytics`,description:`System to analyze sales performance across regions, products, and time periods.`,problemStatement:`Organizations lack visibility into sales performance trends across regions and products.`,architecture:`/architecture/sales-analytics.png`,workflow:[`Sales data ingestion`,`Data cleaning`,`KPI computation`,`Trend analysis`,`Dashboard visualization`],deployment:`Deployed using Power BI and SQL-based data warehouse.`,github:``,demo:``,documentation:``,tech:[`Python`,`SQL`,`Power BI`]},{id:`kpi-monitoring-platform`,title:`Business KPI Monitoring Platform`,category:`Business Intelligence`,description:`Real-time KPI monitoring system for business performance tracking.`,problemStatement:`Businesses need real-time visibility into critical KPIs for faster decision-making.`,architecture:`/architecture/kpi-dashboard.png`,workflow:[`Data ingestion pipelines`,`KPI calculation engine`,`Real-time processing`,`Dashboard updates`,`Alert generation`],deployment:`Deployed using Power BI with SQL backend for real-time analytics.`,github:``,demo:``,documentation:``,tech:[`Power BI`,`Python`,`SQL`]},{id:`marketing-campaign-analysis`,title:`Marketing Campaign Effectiveness Analysis`,category:`Marketing Analytics`,description:`Analytics system to evaluate marketing campaign performance and ROI.`,problemStatement:`Marketing teams struggle to measure campaign effectiveness across channels.`,architecture:`/architecture/marketing-analytics.png`,workflow:[`Campaign data collection`,`Engagement analysis`,`Conversion tracking`,`ROI computation`,`Performance reporting`],deployment:`Deployed using Python analytics pipeline with Power BI dashboards.`,github:``,demo:``,documentation:``,tech:[`Python`,`Excel`,`Power BI`]},{id:`customer-segmentation-analytics`,title:`Customer Segmentation using Data Analytics`,category:`Segmentation`,description:`ML-powered system to segment customers based on behavior and demographics.`,problemStatement:`Businesses need better segmentation strategies for targeted marketing.`,architecture:`/architecture/segmentation.png`,workflow:[`Data preprocessing`,`Feature engineering`,`K-Means clustering`,`Segment analysis`,`Visualization`],deployment:`Deployed as batch analytics pipeline using Python and visualization tools.`,github:``,demo:``,documentation:``,tech:[`Python`,`K-Means`,`Pandas`]},{id:`product-performance-analysis`,title:`Product Performance Analysis System`,category:`Product Analytics`,description:`System to analyze product performance and market trends.`,problemStatement:`Companies lack visibility into product-level performance and trends.`,architecture:`/architecture/product-analytics.png`,workflow:[`Product data ingestion`,`Sales analysis`,`Trend detection`,`Performance ranking`,`Insight reporting`],deployment:`Deployed using Power BI dashboards with SQL backend.`,github:``,demo:``,documentation:``,tech:[`Python`,`Power BI`,`SQL`]},{id:`revenue-forecasting-analytics`,title:`Revenue Forecasting System`,category:`Forecasting`,description:`Time series forecasting system for revenue prediction.`,problemStatement:`Businesses need accurate revenue forecasting for planning and budgeting.`,architecture:`/architecture/revenue-forecast.png`,workflow:[`Historical data collection`,`Time series preprocessing`,`Model training (ARIMA/Prophet)`,`Forecast generation`,`Visualization`],deployment:`Deployed using Python-based forecasting pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`Time Series`,`Prophet`]},{id:`retail-inventory-analytics`,title:`Retail Inventory Optimization System`,category:`Retail Analytics`,description:`Analytics system to optimize inventory levels in retail stores.`,problemStatement:`Retailers face challenges in maintaining optimal inventory levels.`,architecture:`/architecture/inventory.png`,workflow:[`Inventory data ingestion`,`Demand analysis`,`Stock prediction`,`Optimization logic`,`Dashboard reporting`],deployment:`Deployed using SQL + Power BI analytics pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`SQL`,`Power BI`]},{id:`ecommerce-user-journey`,title:`E-commerce User Journey Analysis`,category:`User Analytics`,description:`System to analyze user journey and conversion funnel behavior.`,problemStatement:`E-commerce platforms need better understanding of user drop-off points.`,architecture:`/architecture/user-journey.png`,workflow:[`Clickstream data collection`,`Session tracking`,`Funnel analysis`,`Conversion tracking`,`Insight visualization`],deployment:`Deployed using Python analytics and Google Analytics integration.`,github:``,demo:``,documentation:``,tech:[`Python`,`Google Analytics`,`SQL`]},{id:`churn-analytics-dashboard`,title:`Customer Churn Analytics Dashboard`,category:`Customer Retention`,description:`Analytics system to detect and analyze customer churn patterns.`,problemStatement:`Businesses need to identify customers at risk of churn early.`,architecture:`/architecture/churn.png`,workflow:[`Data preprocessing`,`Feature engineering`,`Churn prediction model`,`Risk scoring`,`Dashboard visualization`],deployment:`Deployed using ML pipeline with Power BI dashboard.`,github:``,demo:``,documentation:``,tech:[`Python`,`Power BI`,`ML`]},{id:`financial-data-insights`,title:`Financial Data Insights System`,category:`Finance Analytics`,description:`System to analyze financial data and detect risk patterns.`,problemStatement:`Financial institutions need automated risk analysis from large datasets.`,architecture:`/architecture/finance.png`,workflow:[`Financial data ingestion`,`Risk modeling`,`Anomaly detection`,`Insight generation`,`Reporting`],deployment:`Deployed using Python analytics and SQL-based storage.`,github:``,demo:``,documentation:``,tech:[`Python`,`SQL`,`Risk Modeling`]},{id:`hr-analytics-dashboard`,title:`HR Analytics Dashboard`,category:`HR Analytics`,description:`System to analyze employee performance and HR metrics.`,problemStatement:`HR teams need insights into employee performance and retention.`,architecture:`/architecture/hr.png`,workflow:[`HR data collection`,`Performance metrics calculation`,`Trend analysis`,`Attrition analysis`,`Visualization`],deployment:`Deployed using Power BI and Python analytics pipeline.`,github:``,demo:``,documentation:``,tech:[`Python`,`Power BI`,`Excel`]}];function P({project:e}){return(0,N.jsx)(M,{to:`/data-science/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/data-science/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function F(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Data Science Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Ri.map(e=>(0,N.jsx)(P,{project:e},e.id))})]})}function zi(){let{id:e}=ot(),t=Ri.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}var Bi=[{id:`ai-research-team`,title:`AI Research Team System`,category:`Multi-Agent Systems`,description:`Collaborative agents automate end-to-end research from data collection to structured reporting.`,problemStatement:`Researchers spend significant time manually searching, summarizing, and structuring information from multiple sources. This system automates research workflows using multiple coordinated agents.`,architecture:`/architecture/multi-agent-research.png`,workflow:[`User submits research query`,`Search agent gathers relevant data`,`Summarizer agent processes raw information`,`Reporter agent structures final insights`,`Final report is generated and delivered`],deployment:`Deployed using FastAPI backend with multi-agent orchestration layer and React UI for interaction.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`Multi-Agent Systems`,`RAG`]},{id:`business-intelligence-system`,title:`Autonomous Business Intelligence System`,category:`Analytics Multi-Agent`,description:`Multi-agent system that automates business intelligence reporting using data, analytics, and insight agents.`,problemStatement:`Business analysts spend excessive time generating reports from raw data. This system automates data-to-insight pipelines using autonomous agents.`,architecture:`/architecture/bi-multi-agent.png`,workflow:[`Data agent collects raw business data`,`Analytics agent processes KPIs`,`Insight agent generates business insights`,`Final BI report is created automatically`],deployment:`Deployed as cloud-based analytics service with API integration and dashboard interface.`,github:``,demo:``,documentation:``,tech:[`Python`,`Pandas`,`LLMs`,`Agentic AI`]},{id:`customer-support-multi-agent`,title:`Customer Support Multi-Agent System`,category:`Enterprise Support Agents`,description:`Multi-agent support system for ticket classification, resolution, and escalation.`,problemStatement:`Customer support teams struggle with high ticket volume and slow resolution times due to manual routing and handling.`,architecture:`/architecture/support-multi-agent.png`,workflow:[`User submits support ticket`,`Triage agent classifies issue`,`Resolution agent attempts automated solution`,`Escalation agent handles complex cases`,`Response sent to user`],deployment:`Integrated into enterprise support portal using FastAPI and chatbot interface.`,github:``,demo:``,documentation:``,tech:[`Python`,`Tool Calling`,`LLMs`,`Agents`]},{id:`devops-monitoring-system`,title:`AI DevOps Monitoring System`,category:`DevOps Multi-Agent`,description:`Automated monitoring system using agents for logs, alerts, and remediation suggestions.`,problemStatement:`DevOps teams face challenges in detecting system failures early and resolving them quickly in complex distributed systems.`,architecture:`/architecture/devops-multi-agent.png`,workflow:[`Log agent collects system logs`,`Monitoring agent detects anomalies`,`Alert agent triggers notifications`,`Fix agent suggests remediation steps`],deployment:`Deployed with Kubernetes-based microservices and monitoring dashboard.`,github:``,demo:``,documentation:``,tech:[`Python`,`Monitoring`,`LLMs`,`DevOps`]},{id:`intelligent-hiring-system`,title:`Intelligent Hiring System`,category:`HR Multi-Agent`,description:`Automated recruitment system using resume parsing, ranking, and interview simulation agents.`,problemStatement:`HR teams spend significant time manually screening resumes and evaluating candidates.`,architecture:`/architecture/hiring-multi-agent.png`,workflow:[`Resume parser extracts candidate data`,`Ranking agent evaluates candidates`,`Interview agent simulates Q&A`,`Final hiring recommendation generated`],deployment:`Deployed as HR automation tool integrated with recruitment portal.`,github:``,demo:``,documentation:``,tech:[`Python`,`NLP`,`LLMs`,`Recruitment AI`]},{id:`financial-analysis-multi-agent`,title:`Financial Analysis Multi-Agent System`,category:`Finance Multi-Agent`,description:`Multi-agent financial system for data analysis, forecasting, and risk evaluation.`,problemStatement:`Financial analysts struggle with analyzing large datasets and generating timely insights.`,architecture:`/architecture/finance-multi-agent.png`,workflow:[`Data agent collects financial data`,`Risk agent evaluates risk factors`,`Forecast agent predicts trends`,`Final financial report generated`],deployment:`Deployed in secure financial analytics environment with API-based access.`,github:``,demo:``,documentation:``,tech:[`Python`,`Finance ML`,`LLMs`,`Agents`]},{id:`ecommerce-optimization`,title:`E-commerce Optimization System`,category:`Business Multi-Agent`,description:`Multi-agent system for pricing, inventory, and recommendation optimization.`,problemStatement:`E-commerce platforms struggle to dynamically optimize pricing, inventory, and recommendations in real time.`,architecture:`/architecture/ecommerce-multi-agent.png`,workflow:[`Pricing agent optimizes product prices`,`Inventory agent tracks stock levels`,`Recommendation agent suggests products`,`System updates e-commerce strategy`],deployment:`Deployed as cloud-based e-commerce optimization engine.`,github:``,demo:``,documentation:``,tech:[`Python`,`Recommender Systems`,`LLMs`]},{id:`smart-travel-planning`,title:`Smart Travel Planning System`,category:`Planning Multi-Agent`,description:`Multi-agent travel system for itinerary creation, booking, and budget optimization.`,problemStatement:`Travel planning requires coordination between itinerary creation, budgeting, and bookings which is time-consuming for users.`,architecture:`/architecture/travel-multi-agent.png`,workflow:[`Itinerary agent creates travel plan`,`Booking agent suggests flights/hotels`,`Budget agent optimizes cost`,`Final travel plan generated`],deployment:`Deployed as web-based travel assistant with API integrations.`,github:``,demo:``,documentation:``,tech:[`Python`,`LLMs`,`APIs`]}];function Vi({project:e}){return(0,N.jsx)(M,{to:`/multi-agent-ai/${e.id}`,style:{textDecoration:`none`,color:`inherit`},onClick:()=>{console.log(`🚀 Project Clicked`),console.log(`Title:`,e.title),console.log(`Navigate To:`,`/multi-agent-ai/${e.id}`)},children:(0,N.jsxs)(`div`,{className:`project-card`,children:[(0,N.jsx)(`h3`,{children:e.title}),(0,N.jsx)(`p`,{children:e.description})]})})}function Hi(){return(0,N.jsxs)(`section`,{className:`projects`,children:[(0,N.jsx)(`h1`,{children:`Multi-Agent Projects`}),(0,N.jsx)(`div`,{className:`projects-grid`,children:Bi.map(e=>(0,N.jsx)(Vi,{project:e},e.id))})]})}function Ui(){let{id:e}=ot(),t=Bi.find(t=>t.id===e);console.log(`📌 Project Page Opened:`,e);let[n,r]=(0,x.useState)([]);if(!t)return console.log(`❌ Project Not Found`),(0,N.jsx)(`h2`,{children:`Project Not Found`});let i=(e,t)=>{let n=`${e} clicked → ${t}`;console.log(`🔗`,n),r(e=>[n,...e])};return(0,N.jsxs)(`div`,{className:`project-container`,children:[(0,N.jsx)(`h1`,{className:`title`,children:t.title}),(0,N.jsx)(`p`,{className:`subtitle`,children:t.description}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📌 Problem Statement`}),(0,N.jsx)(`p`,{children:t.problemStatement})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🏗️ Architecture Diagram`}),(0,N.jsx)(`img`,{src:t.architecture,alt:`Architecture Diagram`,style:{width:`100%`,borderRadius:`10px`}})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔄 End-to-End Workflow`}),(0,N.jsx)(`ol`,{children:t.workflow.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🚀 Deployment`}),(0,N.jsx)(`p`,{children:t.deployment})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`⚙️ Tech Stack`}),(0,N.jsx)(`ul`,{children:t.tech.map(e=>(0,N.jsx)(`li`,{children:e},e))})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`🔗 Resources`}),(0,N.jsx)(`a`,{href:t.github,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`GitHub Repository`,t.github),children:`GitHub Repository`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.demo,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Live Demo`,t.demo),children:`Live Demo`}),(0,N.jsx)(`br`,{}),(0,N.jsx)(`a`,{href:t.documentation,target:`_blank`,rel:`noreferrer`,onClick:()=>i(`Documentation`,t.documentation),children:`Documentation`})]}),(0,N.jsxs)(`div`,{className:`section`,children:[(0,N.jsx)(`h2`,{children:`📊 Activity Logs`}),n.length===0?(0,N.jsx)(`p`,{children:`No interactions yet`}):(0,N.jsx)(`ul`,{children:n.map((e,t)=>(0,N.jsx)(`li`,{children:e},t))})]})]})}function Wi(){return(0,N.jsxs)(At,{children:[(0,N.jsx)(Bn,{}),(0,N.jsxs)(At,{children:[(0,N.jsx)(j,{path:`/`,element:(0,N.jsx)(Kn,{})}),(0,N.jsx)(j,{path:`/projects`,element:(0,N.jsx)(Yn,{})}),(0,N.jsx)(j,{path:`/about`,element:(0,N.jsx)(Jn,{})}),(0,N.jsx)(j,{path:`/books`,element:(0,N.jsx)(Xn,{})}),(0,N.jsx)(j,{path:`/machine-learning`,element:(0,N.jsx)(ei,{})}),(0,N.jsx)(j,{path:`/machine-learning/:id`,element:(0,N.jsx)(ti,{})}),(0,N.jsx)(j,{path:`/deep-learning`,element:(0,N.jsx)(ii,{})}),(0,N.jsx)(j,{path:`/deep-learning/:id`,element:(0,N.jsx)(ai,{})}),(0,N.jsx)(j,{path:`/reinforcement-learning`,element:(0,N.jsx)(ci,{})}),(0,N.jsx)(j,{path:`/reinforcement-learning/:id`,element:(0,N.jsx)(li,{})}),(0,N.jsx)(j,{path:`/mlops`,element:(0,N.jsx)(fi,{})}),(0,N.jsx)(j,{path:`/mlops/:id`,element:(0,N.jsx)(pi,{})}),(0,N.jsx)(j,{path:`/retrieval-augmented-generation`,element:(0,N.jsx)(gi,{})}),(0,N.jsx)(j,{path:`/retrieval-augmented-generation/:id`,element:(0,N.jsx)(_i,{})}),(0,N.jsx)(j,{path:`/llmops`,element:(0,N.jsx)(bi,{})}),(0,N.jsx)(j,{path:`/llmops/:id`,element:(0,N.jsx)(xi,{})}),(0,N.jsx)(j,{path:`/agentic-ai`,element:(0,N.jsx)(wi,{})}),(0,N.jsx)(j,{path:`/agentic-ai/:id`,element:(0,N.jsx)(Ti,{})}),(0,N.jsx)(j,{path:`/natural-language-processing`,element:(0,N.jsx)(Oi,{})}),(0,N.jsx)(j,{path:`/natural-language-processing/:id`,element:(0,N.jsx)(ki,{})}),(0,N.jsx)(j,{path:`/generative-ai`,element:(0,N.jsx)(Mi,{})}),(0,N.jsx)(j,{path:`/generative-ai/:id`,element:(0,N.jsx)(Ni,{})}),(0,N.jsx)(j,{path:`/system-design`,element:(0,N.jsx)(Ii,{})}),(0,N.jsx)(j,{path:`/system-design/:id`,element:(0,N.jsx)(Li,{})}),(0,N.jsx)(j,{path:`/data-science`,element:(0,N.jsx)(F,{})}),(0,N.jsx)(j,{path:`/data-science/:id`,element:(0,N.jsx)(zi,{})}),(0,N.jsx)(j,{path:`/multi-agent-ai`,element:(0,N.jsx)(Hi,{})}),(0,N.jsx)(j,{path:`/multi-agent-ai/:id`,element:(0,N.jsx)(Ui,{})}),(0,N.jsx)(j,{path:`/rag-cookbook`,element:(0,N.jsx)(sr,{})}),(0,N.jsx)(j,{path:`/agentic-cookbook`,element:(0,N.jsx)(vr,{})}),(0,N.jsx)(j,{path:`/multi-agent-cookbook`,element:(0,N.jsx)(kr,{})}),(0,N.jsx)(j,{path:`/mcp`,element:(0,N.jsx)(Br,{})}),(0,N.jsx)(j,{path:`/a2a`,element:(0,N.jsx)(Zr,{})})]})]})}zn.createRoot(document.getElementById(`root`)).render((0,N.jsx)(x.StrictMode,{children:(0,N.jsx)(Wi,{})}));